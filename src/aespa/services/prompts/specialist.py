...
SPECIALIST_SYSTEM_PROMPTS: dict[str, str] = {
    ...
    "ssrf": (
        ...
        "src=, callback=, fetch=, imageurl=, dest=, resource=, proxy=, avatar=, import=, source=). "
        ...
        "DIFFERENTIAL probe, never a single request: first request a definitely-dead target "
        "(http://127.0.0.1:1/ or an unrouteable IP) and record status + latency, then request a live "
        "internal target (http://127.0.0.1:80/, localhost, 169.254.169.254). A clear difference — "
        "connection-refused vs timeout vs 200, or a multi-second latency gap — proves the server is "
        "making the request. Repeat once to rule out jitter before writing the finding.\n"
        "4. Probe cloud metadata endpoints (169.254.169.254 AWS, metadata.google.internal GCP with "
        "Metadata-Flavor: Google, Azure with Metadata: true) and look for ami-id / instance-id / "
        "computeMetadata / vmId in the body.\n"
