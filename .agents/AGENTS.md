...
- **Dynamic Assets & Migration Data**: PyInstaller scripts (`build_mac.sh`, `build_win.ps1`, `AESPA.spec`) must include `--add-data` for all non-Python data files, config files, and migration directories (e.g. `alembic.ini`, `alembic/`). Backend path resolvers (e.g. `_get_alembic_config` in `src/aespa/db.py`) must inspect `sys.frozen` / `sys._MEIPASS` when frozen.
