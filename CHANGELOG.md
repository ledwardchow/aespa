...
- **Light and Deep SAST modes**: Choose Light for the original lower-cost source analysis workflow, or Deep for repository modeling, threat-directed coverage, candidate reconciliation, and semantic closure. The selected mode is saved with the run and kept when it is paused, resumed, rerun, exported, or imported. Resuming a Deep scan continues from its saved phase without repeating completed repository, threat, or planning work. Runs created before this setting was added are migrated as Light.
