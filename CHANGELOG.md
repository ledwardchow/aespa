...
- **Site map search**: Search node URLs, titles, and grouped route variants to highlight matching nodes. A match count updates while typing, and clearing the search removes the highlights. A separate Display APIs control below the search is off by default. Enabling it shows API nodes and unlocks their grouped or individual display setting.

- **Grouped site map routes and APIs**: Site maps now group page URL variants such as `/orders/123` and `/orders/456` into one route while keeping interactive browser states separate. API request variants are grouped by HTTP method and route, and pages and API endpoints use a free-form layout, with configured landing and login pages labelled and enlarged. Selecting a group shows its connections, observed parameters, and URL variants. Individual page and API nodes remain available, along with an API-hidden view. Display controls use a compact toolbar with aligned, theme-matched fields and totals that wrap within the available width. They are collapsed by default while the current node and grouping totals remain visible. Failed crawls and routes that returned only 4xx responses are hidden. Routes discovered by older Dynamic Scans recover source-page links from saved traffic when possible; routes without source evidence are collected under Direct scan discoveries. New scans save these links as routes are observed. SVG and JavaScript files are hidden by default, with an Exclude extensions field for other static file types.
...
- **Site credential form layout**: Login fields are grouped into separate cards with aligned inputs and compact actions. The layout adapts to the available form width, keeping credentials and controls inside the edit site card. The Auth Mode dropdown now matches the other form controls.

- **Run navigation at narrower widths**: Web run navigation switches to a section selector when the tabs no longer fit, with actions kept together. Crawl settings and display controls keep content-sized spacing on desktop and use aligned columns at narrower widths.
