...
- **Grouped site map APIs**: Site maps now group API request variants by HTTP method and route, place pages and API endpoints in separate lanes, and show calling pages, observed parameters, and URL variants when an endpoint is selected. Individual request nodes and an API-hidden view remain available. SVG and JavaScript files are hidden by default, with an Exclude extensions field for other static file types.
