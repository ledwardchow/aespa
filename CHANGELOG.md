...
- **Deep web scans**: Web DAST runs can use a separate Deep mode that builds a saved task queue from crawl evidence, OWASP coverage items, interactive workflows, recon checks, and imported SAST leads. Coverage checks for the same HTTP operation run as one worker campaign instead of separate tasks for every vulnerability class and input. Saved workflows, site-wide recon checks, and SAST leads remain distinct work. The task limit is applied after grouping so one large endpoint does not crowd useful recon or SAST work out of the queue. Workers reuse request and session context across bundled checks and add follow-up probes when the target provides supporting evidence. Several attack workers test the queue in parallel using the existing session, browser replay, scope, traffic, reporting, validation, and per-run token accounting. Their HTTP, browser, and Python probes update the OWASP workprogram while the scan runs, and each task can link to several findings. Stopping a Deep scan returns interrupted tasks to the queue without consuming their retry allowance, and the run offers Resume while completed work stays finished. Selecting a different scan mode changes that action to Start Pentest so a checkpoint is never resumed under the wrong mode. Deep has separate worker and task limits under Agent Settings using the same layout as the other agent settings. Its Activity view groups Deep attack workers into one row that expands to show only currently running workers, labels each worker with its queue purpose, shows specialist and Deep worker history together under Workers, and keeps the saved Work Queue separate. Work Queue shows one expandable row per campaign with its check, input, identity-comparison, and finding counts. Expanding a row shows every bundled check and its status. Finished tasks are labelled Complete. Worker traces retain a readable description for every step, show the request or tool as supporting detail, and restore saved steps after navigation or restart. Deep Scan is hidden by default and can be enabled under System Settings -> Feature Visibility -> Experimental Features. Quick, Standard, Full, SAST Validate, and API scans keep their existing behaviour.
