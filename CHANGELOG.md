...
- **Complete SAST activity history**: The SAST phase log now records process-restart interruptions, including the reason a scan stopped. Agent Activity shows each discovery worker and candidate validator as it is queued, started, completed, blocked, paused, cancelled, or failed. Older runs rebuild available worker activity from their saved work program and validator checkpoints.
- **More reliable Codex tool calls**: Codex scans now correct prose-only responses before they appear as failed tool calls. If Codex reports that a named AESPA tool is unavailable, AESPA replaces that tool session and retries. Unavailable target dependencies, such as a test payment gateway, do not trigger a session reset. Recoverable responses are shown as retries, while errors are reserved for attempts that could not recover.
- **Reliable API login sessions**: API scans no longer treat stored usernames and passwords as if they were already authenticated sessions. Successful login responses can save returned tokens or cookies under a reusable label, including when replacing an expired session. SAST validation runs now report an incomplete result when authentication prevents protected endpoints from being tested.
