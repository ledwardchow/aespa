...
- **More reliable desktop app startup**: The macOS and Windows apps now keep their selected local port reserved until the backend starts. This prevents another process from taking the port during launch, reports startup failures sooner, and gives database upgrades more time to finish on slower systems.
- **Interactive terminal console**: Running AESPA from a terminal now opens a fixed console interface with separate views for server HTTP requests, Python errors, LLM traffic, agent activity, and HTTP traffic sent to test targets. Testing requests are collapsed by default; use the arrow keys and Enter to inspect their request and response headers and bodies. Number keys switch views, Page Up and Page Down browse buffered output without moving a selected historical page, and the page counter, scrollback percentage, and scrollbar show the current position. The layout automatically follows terminal resizing.
