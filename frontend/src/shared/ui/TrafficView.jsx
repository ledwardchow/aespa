...
function readTrafficLayout() {
  try {
    ...
    return {
      detailHeight: Number.isFinite(saved?.detailHeight)
        ? clamp(saved.detailHeight, MIN_DETAIL_HEIGHT, MAX_DETAIL_HEIGHT)
        : null,
      requestWidth: Number.isFinite(saved?.requestWidth)
        ? clamp(saved.requestWidth, MIN_PANE_WIDTH, MAX_PANE_WIDTH)
        : defaultTrafficLayout.requestWidth,
    };
  } catch {
...
export function TrafficTable({
  ...
  const header = (label, field, index, className = "") => (
    <th
    ...
    <table className="traffic-table">
      <colgroup>
        {widths?.map((width, index) => (
          <col key={index} style={{ width: width != null ? width + "px" : undefined }} />
        ))}
      </colgroup>
      <thead>
        <tr>
          {header("#", "_seq", 0, "tr-num")}
      ...
      <tbody>
        {entries.map((entry, index) => (
          <tr
            key={entry.id}
            className={"traffic-row" + (selected?.id === entry.id ? " selected" : "")}
            onClick={() => onSelect(selected?.id === entry.id ? null : entry)}
          >
            <td className="tr-num">{sequenceFor(entry, index)}</td>
            <td className="tr-ts">{formatTime(entry.created_at)}</td>
            <td>
            ...
            <td className="tr-url" title={entry.url}>
              {entry.url}
            </td>
            <td className="tr-dur">{entry.duration_ms != null ? entry.duration_ms + "ms" : "-"}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
export function TrafficDetail({ entry, onClose }) {
    ...
    const onMove = (moveEvent) => {
      if (axis === "y") {
        const nextHeight = clamp(
          startDetailHeight - (moveEvent.clientY - startY),
          (panelHeight * MIN_DETAIL_HEIGHT) / 100,
          (panelHeight * MAX_DETAIL_HEIGHT) / 100,
        );
        updateLayout({ detailHeight: (nextHeight / panelHeight) * 100 });
      } else {
        const nextWidth = clamp(
          startRequestWidth + ((moveEvent.clientX - startX) / availableWidth) * 100,
          MIN_PANE_WIDTH,
          MAX_PANE_WIDTH,
        );
  ...
  const adjustLayout = (axis, direction) => {
    if (axis === "y") {
      updateLayout({
        detailHeight: clamp(
          (layout.detailHeight ?? 50) + direction * 2,
          MIN_DETAIL_HEIGHT,
          MAX_DETAIL_HEIGHT,
        ),
      });
      ...
      updateLayout({
        requestWidth: clamp(layout.requestWidth + direction * 2, MIN_PANE_WIDTH, MAX_PANE_WIDTH),
      });
    }
    saveLayout();
  };
  ...
  const detailHeight =
    layout.detailHeight == null ? undefined : { flex: `0 0 ${layout.detailHeight}%` };
  ...
  return (
    <div className="traffic-detail-wrap" ref={detailWrapRef} style={detailHeight}>
      ...
      <div
        className="traffic-detail-resizer"
        role="separator"
        aria-label="Resize request and response display height"
        aria-orientation="horizontal"
        aria-valuemin={MIN_DETAIL_HEIGHT}
        aria-valuemax={MAX_DETAIL_HEIGHT}
        aria-valuenow={Math.round(layout.detailHeight ?? 50)}
        tabIndex="0"
        onPointerDown={(event) => startResize(event, "y")}
        onKeyDown={(event) => handleKeyDown(event, "y")}
      >
        <span className="traffic-resizer-grip" aria-hidden="true" />
      </div>
      <div
        className="traffic-detail"
        ref={detailRef}
        style={{
          gridTemplateColumns: `minmax(0, ${requestWidth}fr) 8px minmax(0, ${100 - requestWidth}fr)`,
        }}
      >
        <div className="traffic-pane">
          <div className="traffic-pane-label">
            REQUEST — {entry.method} {entry.url}
          </div>
          <pre className="traffic-raw">{rawRequest(entry)}</pre>
        </div>
        <div
          className="traffic-pane-divider"
          role="separator"
        ...
        <div className="traffic-pane">
          <div className="traffic-pane-label">
            ...
            <button
              ...
              onClick={onClose}
              style={{ marginLeft: "auto", paddingRight: "8px" }}
            >
              ✕
            </button>
          </div>
          <pre className="traffic-raw">{rawResponse(entry)}</pre>
        </div>
      </div>
    </div>
  );
