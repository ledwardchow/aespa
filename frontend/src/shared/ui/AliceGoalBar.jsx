export function AliceGoalBar({ goal, running, onPause, onResume, onEdit, onClear }) {
  ...
  const detail =
    goal.pause_reason ||
    goal.blocker ||
    (Array.isArray(remaining) && remaining.length
      ? `${remaining.length} item${remaining.length === 1 ? "" : "s"} remaining`
      : "Working toward verified completion");

  return (
    <div className={`alice-goal-bar alice-goal-bar--${goal.status}`}>
      <div className="alice-goal-copy">
        <span className="alice-goal-status">Goal · {goal.status.replace("_", " ")}</span>
        <span className="alice-goal-objective" title={goal.objective}>
          {goal.objective}
        </span>
      ...
      <div className="alice-goal-actions">
        ...
        {!running && ["paused", "waiting_input"].includes(goal.status) && (
          <button onClick={onResume}>Resume</button>
        )}
        {!running && !["completed", "blocked"].includes(goal.status) && (
