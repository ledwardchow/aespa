...
export function FindingReferenceLink({
  ...
  const originHref = origin?.type === "sast" && origin.run_id && origin.reference
    ? `#/sast-runs/${origin.run_id}/candidates?lead=${encodeURIComponent(origin.reference)}`
    : null;
  ...
  >
    ...
    >{ref}</a> : <button
      ...
      aria-label={`${kind} ${ref}${title ? `: ${title}` : ""}`}
    >{ref}</button>}
    {active && reference && <span className="finding-reference-popover" role="tooltip">
      ...
      {description && <span className="finding-reference-popover-description">{description}</span>}
      {validation_note && <span className="finding-reference-popover-note">Validation: {validation_note}</span>}
      {finding_source && <span className="finding-reference-popover-note">Source: {finding_source}</span>}
      {originLabel && <span className="finding-reference-popover-note">Origin: {originLabel}{origin.reference ? <> · {originHref ? <a href={originHref}>{origin.reference}</a> : origin.reference}</> : ""}</span>}
      {validatedByLabel && <span className="finding-reference-popover-note">Validated by: {validatedByLabel}</span>}
      {runReference && runReference !== reference && <span className="finding-reference-popover-note">Run reference: {runReference}</span>}
    </span>}
  </span>;
}
