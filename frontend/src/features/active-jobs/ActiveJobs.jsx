      ...
      : status === "stopping"
        ...
        : status === "complete"
          ...
          : [
            ...
            : "neutral";
  const label =
    { sast_running: "source scan", correlating: "correlating", dast_running: "live testing" }[
      status
    ] || status;
...
export function ActiveJobsPage() {
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      setJobs(await webRunsApi.listActiveJobs());
    } catch (e) {
      setError(e.message);
    }
  }, []);
    ...
    try {
      if (j.run_type === "campaign") {
        ...
        if (j.job_type === "A.L.I.C.E.") {
          await apiRunsApi.stopApiAliceRun(j.run_id);
        } else {
          await apiRunsApi.stopApiScan(j.run_id);
        }
      } else {
        if (j.job_type === "A.L.I.C.E.") {
          await webRunsApi.stopAliceRun(j.run_id);
        } else if (j.job_type === "Validation") {
          await webRunsApi.stopValidation(j.run_id);
        } else {
          await webRunsApi.stopRun(j.run_id);
        }
      }
      await load();
    } catch (e) {
              ...
              <thead>
                <tr>
                  <th
                    ...
                    onClick={() => toggleSort("run_name")}
                  >
                    Run {sortArrow("run_name")}
                  </th>
                  <th
                    style={{ cursor: "pointer", userSelect: "none" }}
                    onClick={() => toggleSort("site_name")}
                  >
                    App / API / SAST {sortArrow("site_name")}
                  </th>
                  <th
                    style={{ cursor: "pointer", userSelect: "none" }}
                    onClick={() => toggleSort("job_type")}
                  >
                    Job {sortArrow("job_type")}
                  </th>
                  <th
                    style={{ cursor: "pointer", userSelect: "none" }}
                  ...
                  <th
                    style={{ cursor: "pointer", userSelect: "none" }}
                    onClick={() => toggleSort("pages_done")}
                  >
                    Progress {sortArrow("pages_done")}
                  </th>
                  <th
                    style={{ cursor: "pointer", userSelect: "none" }}
                    onClick={() => toggleSort("findings_count")}
                  >
                    Findings {sortArrow("findings_count")}
                  </th>
                  <th
                    style={{ cursor: "pointer", userSelect: "none" }}
                    ...
                    <tr key={key}>
                      <td>
                        ...
                        {j.current_url && (
                          ...
                          >
                            {truncUrl(j.current_url, 54)}
                          </div>
                        )}
                      </td>
                      <td>
                        <a href={activeJobScopeLink(j)}>{activeJobScopeName(j)}</a>
                      </td>
                      <td>{j.job_type}</td>
                      ...
                      <td>
                        ...
                        >
                          <button className="btn secondary sm" onClick={() => nav(runLink)}>
                            Open
                          </button>
                          <button
