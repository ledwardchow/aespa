...
function TextBlock({ label, value, code = false }) {
  ...
  return (
    <div className="finding-detail-block">
      <strong>{label}</strong>
      {code ? (
        <pre className="finding-evidence">{value}</pre>
      ) : (
        <div className="finding-detail-text">{value}</div>
      )}
    </div>
  );
}
...
function FindingDetail({ row }) {
  return (
    <div className="campaign-finding-detail">
      ...
      <div className="campaign-finding-meta-line">
        <span>
          <strong>CVSS:</strong> {row.cvss_score != null ? Number(row.cvss_score).toFixed(1) : "—"}
        </span>
        {row.cvss_vector && <code>{row.cvss_vector}</code>}
        {row.affected_url && (
          <span>
            <strong>URL:</strong> {row.affected_url}
          </span>
        )}
        {row.finding_source && (
          <span>
            <strong>Source:</strong> {row.finding_source}
          </span>
        )}
        {row.origin?.label && (
          <span>
            <strong>Origin:</strong> {row.origin.label}
            {row.origin.reference ? ` · ${row.origin.reference}` : ""}
          </span>
        )}
        {row.validated_by?.label && (
          <span>
            <strong>Validated by:</strong> {row.validated_by.label}
          </span>
        )}
      </div>
      {row.validation_note && (
        <div className="finding-validation-note">
          <strong>Validation ({row.status}):</strong> {row.validation_note}
        </div>
      )}
      ...
      {row.evidence_items?.length > 0 && (
        <TextBlock
          label="Structured evidence"
          value={JSON.stringify(row.evidence_items, null, 2)}
          code
        />
      )}
      ...
      <TextBlock
        ...
        value={row.merged_instances !== "[]" ? row.merged_instances : ""}
        code
      />
...
export function CampaignFindingsTab({ applicationId, campaignId, initialFindingRef }) {
  ...
  const [columnWidths, startColumnResize] = useColResize(
    "colw:campaign-findings:v2",
    [104, 150, 180, 330, 96, 120, 110],
  );
  ...
  const groups = useMemo(() => {
    ...
    return [...grouped.values()].sort((a, b) =>
      (a.items[0].severity || "").localeCompare(b.items[0].severity || ""),
    );
  }, [rows]);
