...
import { useCampaignWizardData } from "./useCampaignWizardData.js";
import { formatBytes, shortHash } from "../../shared/runs/campaignPresentation.js";
...
export function CampaignNewForm({ applicationId }) {
  ...
  useEffect(() => {
    ...
    const choices = {};
    eligible.forEach((c) => {
      choices[c.id] = c.latest_snapshot.id;
    });
  ...
  const sourceMembers = useMemo(
    () =>
      [...includedComponents]
        ...
        .filter((m) => m.snapshot_id),
    [includedComponents, snapshotChoice],
  );

  const targetMembers = useMemo(
    () => [...includedTargets].map((id) => ({ target_id: id })),
    [includedTargets],
  );
          ...
          >
            <div className="field">
              ...
              <input
                ...
                placeholder="e.g. August release"
                autoFocus
              />
            </div>
            ...
            <div>
              ...
              {missingSnapshotComponents.length > 0 && (
                <div className="alert warning" style={{ marginBottom: 10 }}>
                  {missingSnapshotComponents.map((c) => c.name).join(", ")}{" "}
                  {missingSnapshotComponents.length === 1 ? "has" : "have"} no uploaded snapshot and
                  cannot be included.
                </div>
              )}
              {components
                ...
                .map((c) => (
                  <div key={c.id} className="app-wizard-row">
                    <label className="row" style={{ gap: 8, flex: 1 }}>
                      <input
                        type="checkbox"
                        checked={includedComponents.has(c.id)}
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setSnapshotChoice((prev) => ({ ...prev, [c.id]: +e.target.value }))
                      }
                    >
                      {(snapshotsByComponent[c.id] || [c.latest_snapshot]).map((s) => (
                        <option key={s.id} value={s.id}>
                          {s.filename} · {shortHash(s.sha256)} · {formatBytes(s.size_bytes)}
                          {s.id === c.latest_snapshot.id ? " (latest)" : ""}
                        </option>
                      ))}
                    </select>
                  </div>
            ...
            <div>
              ...
              {targets.map((t) => (
                <label key={t.id} className="app-wizard-row" style={{ gap: 8 }}>
                  <input
                    type="checkbox"
                    checked={includedTargets.has(t.id)}
                    onChange={() => toggleTarget(t.id)}
                  />
                  <span className="badge neutral">
                    {t.target_type === "site" ? "Site" : "API collection"}
                  </span>
            ...
            <div>
              ...
              <div className="row" style={{ gap: 20, flexWrap: "wrap", marginTop: 10 }}>
                <div className="field" style={{ minWidth: 240 }}>
                  <label>
                    LLM profile <span className="subtle">(optional)</span>
                  </label>
                  <select
                    className="select"
                    value={llmProfileId}
                ...
                <div className="field" style={{ minWidth: 180 }}>
                  ...
                  <input
                    ...
                    max={8}
                    value={maxParallel}
                    onChange={(e) => setMaxParallel(Math.min(8, Math.max(1, +e.target.value || 1)))}
                  />
                </div>
                <div className="field" style={{ minWidth: 180 }}>
                  <label>
                    Trace edges <span className="subtle">(optional)</span>
                  </label>
                  <input
                    type="number"
                    min={1}
                    max={64}
                    value={traceEdges}
                    onChange={(e) => setTraceEdges(e.target.value)}
                    placeholder="Global default"
                  />
                </div>
                <div className="field" style={{ minWidth: 180 }}>
                  <label>
                    Trace components <span className="subtle">(optional)</span>
                  </label>
                  <input
                    type="number"
                    min={1}
                    max={32}
                    value={traceComponents}
                    onChange={(e) => setTraceComponents(e.target.value)}
                    placeholder="Global default"
                  />
                </div>
                <div className="field" style={{ minWidth: 180 }}>
                  <label>
                    Paths per lead <span className="subtle">(optional)</span>
                  </label>
                  <input
                    type="number"
                    min={1}
                    max={100}
                    value={tracePaths}
                    onChange={(e) => setTracePaths(e.target.value)}
                    placeholder="Global default"
                  />
                </div>
                <div className="field" style={{ minWidth: 180 }}>
                  <label>
                    Min trace confidence <span className="subtle">(optional)</span>
                  </label>
                  <input
                    type="number"
                    min={0}
                    max={1}
                    step={0.05}
                    value={traceConfidence}
                    onChange={(e) => setTraceConfidence(e.target.value)}
                    placeholder="Global default"
                  />
                </div>
              </div>
...
function ReviewStep({
  ...
  return (
    ...
    >
      ...
      {created && (
        <div className="alert warning">
          This campaign was already created (draft) — starting it failed. Retrying below only starts
          that same draft; it never creates a second campaign. Editing the selection above no longer
          applies to it — open the draft if you need to change anything.
        </div>
      )}
      <div>
        <strong>{name}</strong>
      ...
      <div>
        <div className="subtle" style={{ fontWeight: 700, marginBottom: 6 }}>
          Code snapshots ({sourceMembers.length})
        </div>
        {sourceMembers.map((m) => (
          <div key={m.component_id} className="app-wizard-row">
            <span>{componentName(m.component_id)}</span>
          </div>
        ))}
      </div>
      <div>
        <div className="subtle" style={{ fontWeight: 700, marginBottom: 6 }}>
          Live targets ({targetMembers.length})
        </div>
        {targetMembers.map((m) => {
          const t = targetById(m.target_id);
          return (
            <div key={m.target_id} className="app-wizard-row">
              <span className="badge neutral">
                {t?.target_type === "site" ? "Site" : "API collection"}
              </span>
      ...
      <div className="subtle">
        LLM profile:{" "}
        {llmProfile
          ? `${llmProfile.name}${llmProfile.default_model_name ? ` · ${llmProfile.default_model_name}` : ""}`
          : "global active profile"}{" "}
        · Max parallel scans: {maxParallel} · Trace overrides:{" "}
        {Object.values(traceOverrides).some(Boolean) ? "custom" : "global defaults"}
      </div>
      <div className="row spread">
        <button
          type="button"
        ...
        <div className="row" style={{ gap: 8 }}>
          {created && (
            <button type="button" className="btn secondary" disabled={saving} onClick={onOpenDraft}>
              Open draft
            </button>
          )}
          <button type="button" className="btn" disabled={saving} onClick={onCreate}>
            {saving ? "Starting…" : created ? "Retry start" : "Create & start campaign"}
          </button>
        </div>
      </div>
    </div>
