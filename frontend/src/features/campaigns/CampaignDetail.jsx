...
import { CampaignComponentsTab } from "./CampaignComponentsTab.jsx";
import { CampaignConnectionsTab } from "./CampaignConnectionsTab.jsx";
import { CampaignReviewTab } from "./CampaignReviewTab.jsx";
import { CampaignRunsTab } from "./CampaignRunsTab.jsx";
import { CampaignFindingsTab } from "./CampaignFindingsTab.jsx";
import { CampaignActivityTab } from "./CampaignActivityTab.jsx";
...
export function CampaignDetail({ applicationId, campaignId, initialTab, initialFindingRef }) {
  ...
  const hasCancelledSource = (campaign.source_members || []).some(
    (member) => member.run_status === "cancelled",
  );
  const canResume =
    ["stopped", "interrupted"].includes(campaign.status) ||
    (campaign.status === "awaiting_review" && hasCancelledSource);
  const canContinue =
    campaignDisplayStatus(campaign) === "awaiting_review" && !!campaign.review_submitted_at;
      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            {canStop && (
              <button className="btn danger-outline" disabled={busy} onClick={stop}>
                {busy ? "Stopping…" : "Stop"}
              </button>
            )}
            {canResume && (
              <button className="btn" disabled={busy} onClick={resume}>
                {busy ? "Resuming…" : "Resume campaign"}
              </button>
            )}
            {canContinue && (
              <button className="btn" disabled={busy} onClick={continueToLive}>
                {busy ? "Starting…" : "Continue to live testing"}
              </button>
            )}
          </>
        }
