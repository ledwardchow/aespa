...
export function ApiRunLogTab({ runId, scanRunning }) {
  ...
  const load = useCallback(
    () =>
      apiRunsApi
        .getApiAgentLogPage(runId, { limit: LOG_PAGE_SIZE })
        .then((page) => {
  ...
  const loadEarlier = async () => {
    ...
    setError(null);
    try {
      const page = await apiRunsApi.getApiAgentLogPage(runId, {
        limit: LOG_PAGE_SIZE,
        beforeId: firstId,
      });
  ...
  const onClear = async () => {
    ...
    setError(null);
    try {
      await apiRunsApi.clearApiAgentLog(runId);
      setLog([]);
      setHasEarlier(false);
      pageLoadedRef.current = false;
    } catch (e) {
    ...
    >
      <div className="activity-log-toolbar">
        <span className="activity-count-label">
          {log.length}
          {hasEarlier ? "+" : ""} entr{log.length !== 1 ? "ies" : "y"}
        </span>
        {scanRunning && <span className="activity-mode-badge">Scan running</span>}
        <a className="btn ghost sm" href={`/api/api-test-runs/${runId}/agent-log/export`} download>
          Export log ↓
        </a>
        {log.length > 0 && (
          <button className="btn danger-outline sm" disabled={clearBusy} onClick={onClear}>
      ...
      {log.length === 0 ? (
        ...
        >
          {scanRunning
            ? "Scan in progress — agent activity will appear here."
          ...
          {log.map((r) => {
            ...
            return (
              <div key={r.id} className="activity-entry">
                ...
                <span className={"activity-badge " + statusCls(r.status)}>
                  {(r.status || "").toUpperCase() || "—"}
                </span>
                <span className="activity-url mono">
                  {r.role} ({r.agent_id})
                </span>
