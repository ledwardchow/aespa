...
import { OWASP_LABELS, COVERAGE_CATEGORIES } from "../../shared/runs/coverage.js";
...
export function ApiRunWorkProgramTab({ runId, scanRunning, run }) {
  ...
  if (!matrix || !matrix.endpoints?.length)
    return (
      <div
        className="subtle"
        style={{
  ...
  const onExportMarkdown = () => {
    ...
    downloadTextFile(
      `${slugForFilename(run?.name || `api-run-${runId}`)}-owasp-coverage-${new Date().toISOString().slice(0, 10)}.md`,
      md,
              ...
              return (
                ...
                >
                  ...
                  {cats.map((cat) => {
                    ...
                    const isSelected =
                      selectedCell?.endpoint_id === ep.endpoint_id && selectedCell?.cat === cat;
                    return (
                      <td
                        key={cat}
                        style={{
                      ...
                      >
                        <span
                          ...
                          title={
                            ...
                            cell.status +
                            (fids.length
                              ? " (" + fids.length + " finding" + (fids.length > 1 ? "s" : "") + ""
                              : "")
                          }
                          style={{
      ...
      {selectedCell && (
        ...
        >
          ...
          >
            ...
            >
              {selectedCell.method} {selectedCell.path} — {selectedCell.cat}{" "}
              {OWASP_LABELS[selectedCell.cat] || ""}
            </b>
