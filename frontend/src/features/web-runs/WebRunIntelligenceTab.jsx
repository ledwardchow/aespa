...
import { TargetIntelligencePanel } from "./TargetIntelligencePanel.jsx";

const totalItems = (data) =>
  Object.values(data?.counts || {}).reduce((total, count) => total + count, 0);
...
export function WebRunIntelligenceTab({ runId, active, captureActive, onTotalChange }) {
  ...
  const loadIntel = useCallback(async () => {
    const result = await webRunsApi.getTargetIntelligence(runId, selectedKind);
    setData(result);
  ...
  const clear = useCallback(async () => {
    ...
    setError(null);
    try {
      await webRunsApi.clearTargetIntel(runId);
      setData(null);
      setSelectedKind("");
      onTotalChange(0);
    } catch (loadError) {
