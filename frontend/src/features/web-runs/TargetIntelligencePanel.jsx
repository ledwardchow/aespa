...
export function TargetIntelligencePanel({
  ...
  const compactMeta = (meta) => {
    ...
    const shown = Object.entries(meta)
      ...
      .slice(0, 5)
      .map(
        ([k, v]) => `${k}: ${Array.isArray(v) ? v.length + " item(s)" : String(v).slice(0, 80)}`,
      );
    return shown.join(" · ");
  };
  return (
      ...
      <div className="intel-toolbar">
        <div className="intel-title">
          ...
          <span className="subtle">
            {total} item{total === 1 ? "" : "s"} discovered during crawl
          </span>
        </div>
        <div className="intel-filter">
          ...
          {total > 0 && (
            <button className="btn danger-outline sm" disabled={clearing} onClick={onClear}>
      ...
      <div className="intel-counts">
        {Object.entries(counts)
          ...
          .map(([kind, count]) => (
            <button
              key={kind}
              className={"intel-count-card" + (selectedKind === kind ? " active" : "")}
              onClick={() => onKind(selectedKind === kind ? "" : kind)}
            >
        ...
        {total === 0 && (
          <div className="subtle">
            No target intelligence has been collected yet. Start or restart a crawl to populate the
            inventory.
          </div>
        )}
      </div>
        ...
        <table className="intel-table">
          ...
          <thead>
            <tr>
              <th>
                ...
                <div
                  className="col-rh"
                  onMouseDown={(e) => startIntelResize(0, e)}
              ...
              <th>
                ...
                <div
                  className="col-rh"
                  onMouseDown={(e) => startIntelResize(1, e)}
                  onClick={(e) => e.stopPropagation()}
                />
              </th>
              <th>
                ...
                <div
                  className="col-rh"
                  onMouseDown={(e) => startIntelResize(2, e)}
                  onClick={(e) => e.stopPropagation()}
                />
              </th>
              <th>
                ...
                <div
                  className="col-rh"
                  onMouseDown={(e) => startIntelResize(3, e)}
                  onClick={(e) => e.stopPropagation()}
                />
              </th>
              <th>
                ...
                <div
                  className="col-rh"
                  onMouseDown={(e) => startIntelResize(4, e)}
                  onClick={(e) => e.stopPropagation()}
                />
              </th>
              <th>
                ...
                <div
                  className="col-rh"
                  onMouseDown={(e) => startIntelResize(5, e)}
                  onClick={(e) => e.stopPropagation()}
                />
              </th>
            </tr>
          ...
          <tbody>
            {items.map((item) => (
              <tr key={item.id}>
                <td>
                  <span className="intel-kind">{kindLabel(item.kind)}</span>
                </td>
                <td>
                  <span className="mono">{item.method || "-"}</span>
                </td>
                <td>
                  <div className="intel-primary" title={item.key}>
                    {item.key || "—"}
                  </div>
                  {item.url && (
                    <div className="intel-url mono" title={item.url}>
                      {truncUrl(item.url, 72)}
                    </div>
                  )}
                </td>
                <td>
                  <div className="intel-value" title={item.value}>
                    {item.value || "—"}
                  </div>
                  {item.evidence && <div className="intel-evidence">{item.evidence}</div>}
                  {compactMeta(item.item_metadata) && (
                    <div className="intel-meta">{compactMeta(item.item_metadata)}</div>
                  )}
                </td>
                <td>{item.source}</td>
