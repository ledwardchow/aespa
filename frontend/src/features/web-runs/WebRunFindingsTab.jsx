      ...
      <div className="findings-panel">
        <div className="findings-status-bar">
          <div className="findings-status-summary">
            {thinkingStatus && thinkingStatus.status && thinkingStatus.status !== "idle" && (
              <span
                className={
                  "scan-status-badge scan-status-" +
                  (thinkingStopRequested ? "stopping" : thinkingStatus.status)
                }
              >
                {thinkingStopRequested
                  ? "Stopping Dynamic Scan…"
                  : thinkingStatus.run_outcome === "incomplete"
                    ? "Validation incomplete — resume available"
                    : thinkingStatus.status === "running"
                      ? "Dynamic Scan running…"
                      : thinkingStatus.status === "analysing" ||
                          thinkingStatus.status === "analyzing"
                        ? "Dynamic Scan analysing…"
                        : thinkingStatus.status === "stopping"
                          ? "Dynamic Scan stopping…"
                          : thinkingStatus.status === "complete"
                            ? "Dynamic Scan complete"
                            : thinkingStatus.status === "stopped"
                              ? "Dynamic Scan stopped"
                              : thinkingStatus.status === "failed"
                                ? "Dynamic Scan failed"
                                : "Dynamic Scan"}
              </span>
            )}
            {validateStatus?.status === "running" ? (
              <span className="val-status-badge val-running">
                Validating…{" "}
                {validateStatus.confirmed +
                  validateStatus.false_positives +
                  (validateStatus.unconfirmed || 0)}
                /{validateStatus.total}
              </span>
            ) : validateStatus?.status === "stopped" ? (
              <span className="val-status-badge val-fp">Validation stopped</span>
            ) : validateStatus?.status === "complete" ? (
              <span className="val-status-badge val-complete">
                {validateStatus.confirmed} confirmed · {validateStatus.unconfirmed || 0} unconfirmed
                · {validateStatus.false_positives} low confidence
              </span>
            ) : null}
            {validateStatus?.status === "running" && (
              <button className="btn danger-outline sm" onClick={onStopValidation}>
                Stop validation
              </button>
            )}
            {dedupeBusy && (
            ...
            {findings.length > 0 && (
              <button
                ...
                onClick={async () => {
                  if (
                    !confirm(
                      "Clear all findings and reset page scan status?\nThis lets you re-run the scanner on the same crawl.",
                    )
                  )
                    return;
                  ...
                  setClearError(null);
                  try {
                    await webRunsApi.clearFindings(runId);
                    setFindings([]);
                  } catch (e) {
              ...
              >
                {clearBusy === "findings" ? "Clearing…" : "Clear all"}
              </button>
            )}
          </div>
        </div>
        {findings.length === 0 ? (
          ...
          <div className="findings-table-wrap">
            {(() => {
              ...
              const makeGroups = (map) =>
                Object.entries(map)
                  .map(([title, items]) => {
                    ...
                    const topSev = items.reduce(
                      (b, f) =>
                        (SEV_ORDER[f.severity] ?? 99) < (SEV_ORDER[b] ?? 99) ? f.severity : b,
                      items[0].severity,
                    );
                    return {
                      title,
              ...
              const deterministicCount = deterministicGroups.reduce(
              ...
              const evidenceItemsFor = (f) => {
                ...
                return items.filter(
                  (it) =>
                    it && it.type !== "validation_verdict" && it.type !== "validation_reasoning",
                );
              };
              const renderFinding = (f, keyPrefix = "") => (
                <React.Fragment key={keyPrefix + f.id}>
                  <tr
                    key={keyPrefix + f.id}
                    className="finding-instance-row"
                    onClick={() => {
                  ...
                  >
                    <td className="finding-reference-cell">
                      ...
                      {f.validation_status === "skipped" && (
                        <span className="val-badge val-skipped">not validated</span>
                      )}
                      {f.validation_status === "false_positive" && (
                      ...
                      {f.validation_status === "validating" && (
                        <span className="val-badge val-validating">…</span>
                      )}
                    </td>
                    <td></td>
                    <td colSpan="2">
                      <span className="instance-chevron">
                        {expandedFinding === f.id ? "▾" : "▸"}
                      </span>
                      <span
                        className="finding-affected-label"
                        style={{
                          marginRight: 6,
                        }}
                      >
                    ...
                    <td>
                      ...
                      >
                        {(f.validation_status === "unvalidated" ||
                          f.validation_status === "skipped" ||
                          f.validation_status === "unconfirmed" ||
                          f.validation_status === "false_positive" ||
                          f.validation_status === "low_confidence") && (
                          <button
                            ...
                            title="Validate"
                            onClick={(e) => onValidateFinding(e, f.id)}
                          >
                            ✓
                          </button>
                        )}
                        ...
                        <button
                          className="btn ghost sm finding-del-btn"
                          title="Delete"
                          onClick={(e) => onDeleteFinding(e, f.id)}
                        >
                          🗑
                        </button>
                      </div>
                  ...
                  {expandedFinding === f.id && editingFinding !== f.id && (
                    <tr key={"ev-" + keyPrefix + f.id} className="finding-evidence-row">
                      <td colSpan="5">
                          ...
                          >
                            ...
                            {f.poc_setup && (
                              ...
                              >
                                ...
                                >
                                  Setup (capture an authenticated session)
                                </summary>
                                <pre
                        ...
                        {evidenceItemsFor(f).length > 0 && (
                          <div className="structured-evidence">
                            {evidenceItemsFor(f).map((item, idx) => (
                              <div
                                key={idx}
                                className={
                                  "structured-evidence-item evidence-type-" + (item.type || "note")
                                }
                              >
                        ...
                        {(() => {
                          ...
                          return (
                            <>
                              ...
                              {!f.request_evidence && !f.response_evidence && f.evidence && (
                        ...
                        {f.screenshot_b64 && (
                          <div className="finding-screenshot-wrap">
                            ...
                            <img
                              src={"data:image/png;base64," + f.screenshot_b64}
                              ...
                              {instances.map((inst, idx) => (
                                ...
                                >
                                  ...
                                  {!inst.request_evidence &&
                                    !inst.response_evidence &&
                                    inst.evidence && (
                                      <pre className="finding-evidence">{inst.evidence}</pre>
                                    )}
                                </div>
                              ))}
                            </div>
              ...
              const renderStatusRows = (statusGroups, keyPrefix) =>
                statusGroups.map((g) => {
                  ...
                  return (
                    <React.Fragment key={groupKey}>
                      ...
                      >
                        <td>
                          <span className={"sev-badge sev-" + g.topSev}>{g.topSev}</span>
                        </td>
                        <td>
                          <span className="source-badge">{sourceLabel(g.source)}</span>
                        </td>
                        ...
                        <td>
                          <span className="finding-count-badge">{g.count}</span>
                        </td>
                        <td></td>
                      </tr>
                      {expandedGroups.has(groupKey) &&
                        g.items.map((f) => renderFinding(f, keyPrefix + "-"))}
                    </React.Fragment>
                  );
                });
                ...
                <table className="findings-table">
                  ...
                  <thead>
                    <tr>
                      <th>
                        ...
                        <div
                          className="col-rh"
                          onMouseDown={(e) => startFindResize(0, e)}
                      ...
                      <th>
                        ...
                        <div
                          className="col-rh"
                          onMouseDown={(e) => startFindResize(1, e)}
                          onClick={(e) => e.stopPropagation()}
                        />
                      </th>
                      <th>
                        ...
                        <div
                          className="col-rh"
                          onMouseDown={(e) => startFindResize(2, e)}
                          onClick={(e) => e.stopPropagation()}
                        />
                      </th>
                      <th>
                        ...
                        <div
                          className="col-rh"
                          onMouseDown={(e) => startFindResize(3, e)}
                          onClick={(e) => e.stopPropagation()}
                        />
                      </th>
                      <th></th>
                  ...
                  <tbody>
                        ...
                        <tr className="finding-group-row" onClick={() => toggleGroup(g.title)}>
                          ...
                          <td>
                            <button
                              className="btn ghost sm finding-del-btn"
                              title="Delete group"
                              onClick={(e) => onDeleteFindingGroup(e, g.title)}
                            >
                              🗑
                            </button>
                          </td>
                    ...
                    {unconfirmedCount > 0 && (
                      <>
                        ...
                        >
                          ...
                          <td className="finding-title">
                            <span className="group-chevron">
                              {expandedGroups.has(UNCONFIRMED_GROUP_KEY) ? "▾" : "▸"}
                            </span>
                            Unconfirmed Findings
                          </td>
                          <td>
                            <span className="finding-count-badge">{unconfirmedCount}</span>
                          </td>
                          <td></td>
                        </tr>
                    ...
                    {fpCount > 0 && (
                      <>
                        ...
                        >
                          ...
                          <td className="finding-title">
                            <span className="group-chevron">
                              {expandedGroups.has(FP_GROUP_KEY) ? "▾" : "▸"}
                            </span>
                            Low Confidence
                          </td>
                          <td>
                            <span className="finding-count-badge">{fpCount}</span>
                          </td>
                          <td></td>
                        </tr>
                    ...
                    {deterministicCount > 0 && (
                      <>
                        <tr
                          key={DETERMINISTIC_GROUP_KEY}
                          className="finding-group-row"
                          onClick={() => toggleGroup(DETERMINISTIC_GROUP_KEY)}
                        >
                          <td>
                            <span className="val-badge val-fp">deterministic</span>
                          </td>
                          <td></td>
                          <td className="finding-title">
                            <span className="group-chevron">
                              {expandedGroups.has(DETERMINISTIC_GROUP_KEY) ? "▾" : "▸"}
                            </span>
                            Deterministic Findings
                          </td>
                          <td>
                            <span className="finding-count-badge">{deterministicCount}</span>
                          </td>
                          <td></td>
                        </tr>
