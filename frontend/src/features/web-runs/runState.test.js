...
import {
  ...
  RUN_PRIMARY_ACTION,
  runStatusFromThinkingStatus,
} from "./runState.js";

test("only resumes when the selected scan mode matches the saved mode", () => {
  assert.equal(canResumeSelectedScanMode(true, "deep", "deep"), true);
  assert.equal(canResumeSelectedScanMode(true, "track", "deep"), false);
  assert.equal(canResumeSelectedScanMode(true, "deep", "track"), false);
...
test("the combined run action starts with crawling when no crawl result exists", () => {
  assert.equal(
    resolveRunPrimaryAction({
      hasCrawlResult: false,
      canStartCrawl: true,
      canStartPentest: true,
      canResumePentest: true,
    }),
...
test("dynamic scan terminal events update the parent run status", () => {
  ...
  assert.equal(
    runStatusFromThinkingStatus({ status: "complete", run_outcome: "incomplete" }, "running"),
    "incomplete",
  );
