...
const isHidden404Node = (node) =>
  node?.status === "failed" &&
  String(node.error_message || "")
    .trim()
    .toUpperCase() === "HTTP 404";
      ...
      if (evt.type === "page_added") {
        setGraph((prev) => {
          ...
          const node = {
            ...
            analysis_status:
              cachedStatus ||
              evt.node.analysis_status ||
              (evt.node.status === "redirect"
                ? "skipped"
                : evt.node.context
                  ? "complete"
                  : "pending"),
          ...
          if (isHidden404Node(nextNode)) {
            return {
              ...
              links: prev.links.filter(
                (link) => link.source !== evt.page_id && link.target !== evt.page_id,
              ),
            };
          }
          return {
        ...
        setAgents((prev) => {
          ...
          const crawlEvent = {
            ...
            stageLabel:
              evt.stage_label || (evt.done ? "Credential phase complete" : "Opening page"),
          ...
          const phaseLabel =
            evt.phase_index && evt.phase_total
              ? `Phase ${evt.phase_index}/${evt.phase_total} · `
              : "";
        ...
        setGuidedLoginPending((prev) =>
          prev.map((p) =>
            p.credential_id === evt.credential_id
              ? {
                  ...p,
                  browserOpen: true,
                }
        ...
        setEntraPrompts((prev) => {
          ...
          const next = [
            prompt,
            ...prev.filter((p) => p.id !== id && p.credential_id !== evt.credential_id),
          ];
