...
import {
  getAliceSession,
  aliceSessionSubscribe,
  aliceSessionAbort,
  aliceSessionConnect,
  aliceSessionStart,
} from "../../shared/alice/session.js";
...
export function useAliceChat(runId, { onActivate } = {}) {
  ...
  const ALICE_WELCOME_MESSAGE =
    "Hello! I am A.L.I.C.E, your interactive pentesting partner. To start a test, click Start Pentest at the top right, or you can tell me to work on something specific!";
    ...
    webRunsApi
      ...
      .then((data) => {
        ...
        const applyRecovery = (chats, tabId) => {
          try {
            const rec = JSON.parse(
              localStorage.getItem(`alice_recover_web:${runId}:${tabId}`) || "null",
            );
            ...
            return chats.map((tab) => {
              ...
              return {
                ...
                messages: tab.messages.map((m) => {
                  if (m.id === rec.thinkMsgId && rec.thought)
                    return {
                      ...m,
                      text: rec.thought,
                      stepData: rec.stepData || m.stepData || {},
                    };
                  if (m.id === rec.replyMsgId && rec.message)
                    return {
                      ...m,
                      text: rec.message,
                    };
        ...
        const localSavedAt = parseInt(
          localStorage.getItem(`alice_chats_${runId}_savedAt`) || "0",
          10,
        );
    ...
    if (!recThinkId || (!recThought && !recMessage)) {
      try {
        const saved = JSON.parse(
          localStorage.getItem(`alice_recover_web:${runId}:${activeAliceTabId}`) || "null",
        );
      ...
      setAliceChats((prev) =>
        prev.map((tab) => {
          ...
          return {
            ...tab,
            messages: tab.messages.map((m) => {
              if (m.id === recThinkId && recThought)
                return {
                  ...m,
                  text: recThought,
                  stepData: recStepData || m.stepData || {},
                };
              if (m.id === recReplyId && recMessage)
                return {
                  ...m,
                  text: recMessage,
                };
    ...
    const unsub = aliceSessionSubscribe(identity, activeAliceTabId, {
      onChunk: (event) => {
        ...
        if (event.goal?.tab_id) {
          setAliceChats((prev) =>
            prev.map((tab) => (tab.id === event.goal.tab_id ? { ...tab, goal: event.goal } : tab)),
          );
        }
          ...
          prev.map((tab) => {
            ...
            return {
              ...
              messages: tab.messages.map((m) => {
                ...
                if (
                  ["step_llm_call", "step_tool_call", "step_tool_result"].includes(event.type) &&
                  m.id === thinkMsgId
                )
                  return {
                    ...m,
  ...
  useEffect(() => {
    ...
    _aliceSaveTimer.current = setTimeout(() => {
      webRunsApi
        .saveAliceSessions(capturedRunId, {
          chats: capturedChats,
  ...
  const handleAliceStop = () => {
    ...
    webRunsApi.stopAliceRun(runId).catch(() => {});
    setAliceThinkingTabId(null);
    setAliceGlobalRunning(false);
    setAliceChats((prev) =>
      prev.map((tab) =>
        tab.id === activeAliceTabId && tab.goal
          ? {
              ...tab,
  ...
  const submitAliceDirective = (rawText, { fromInput = false, onComplete = null } = {}) => {
    ...
    if (aliceIsThinking) {
      ...
      setAliceChats((prev) =>
        prev.map((tab) =>
          tab.id === activeAliceTabId ? { ...tab, messages: [...tab.messages, steerMessage] } : tab,
        ),
      );
      if (fromInput) setAliceInputText("");
      webRunsApi.steerAliceGoal(runId, { message: userText }).catch(() => {});
      return;
    }
    if (fromInput) setAliceInputText("");
    ...
    if (/^\/goal\s+resume$/i.test(userText)) {
      ...
      setAliceChats((prev) =>
        prev.map((tab) => (tab.id === currentTabId ? { ...tab, goal: null } : tab)),
      );
    }
  ...
  const handleAliceSend = () =>
    submitAliceDirective(aliceInputText, {
      fromInput: true,
    });
