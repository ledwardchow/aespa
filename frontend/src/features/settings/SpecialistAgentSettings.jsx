...
import { specialistAgentPayload, specialistAgentToForm } from "./specialistForm.js";
...
export function SpecialistAgentSettings() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setForm(specialistAgentToForm(await settingsApi.getSpecialistAgentConfig()));
      } catch (e) {
        setError(e.message);
      }
    })();
  ...
  const onSubmit = async (e) => {
    ...
    setSaved(false);
    try {
      const savedConfig = await settingsApi.upsertSpecialistAgentConfig(
        specialistAgentPayload(form),
      );
      ...
      {form && (
        <form className="card" onSubmit={onSubmit}>
          ...
          >
            ...
            specific vulnerability leads in parallel. Each specialist receives an independent LLM
            session and a subset of tools (HTTP, browser, context, write_finding).
          </div>
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={form.auto_dispatch_enabled}
              disabled={dis}
              onChange={(e) =>
                upd({
          ...
          >
            Automatically hands off clear upload, SSRF, SQL error, and reflected XSS signals. The
            Test Lead can still dispatch other leads.
          </div>
          ...
          <div className="field">
            ...
            <div className="field-hint">
              Maximum number of specialist agents running at the same time (0 = effectively
              disabled). Default: 5.
            </div>
          </div>
          <div className="field">
            ...
            />
            <div className="field-hint">
              Keeps eligible handoffs until a specialist slot becomes available. Default: 20.
            </div>
          </div>
          <div className="field">
            ...
            />
            <div className="field-hint">
              Step budget for each specialist agent before it is stopped. Default: 30.
            </div>
          </div>
          <div className="field">
            ...
            />
            <div className="field-hint">
              Only dispatch a specialist if the lead's priority score meets this threshold (1–10).
              Default: 7.
            </div>
          </div>
          ...
          >
            Only dispatch specialists for the selected vulnerability classes. Disable classes you
            don't need to keep token usage under control.
          </div>
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={form.dispatch_idor}
              disabled={dis}
              onChange={(e) =>
                upd({
          ...
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={form.dispatch_auth_bypass}
              disabled={dis}
              onChange={(e) =>
                upd({
          ...
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={form.dispatch_business_logic}
              disabled={dis}
              onChange={(e) =>
                upd({
          ...
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={form.dispatch_path_traversal}
              disabled={dis}
              onChange={(e) =>
                upd({
          ...
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={form.dispatch_file_upload}
              disabled={dis}
              onChange={(e) =>
                upd({
          ...
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={form.trigger_specialist_on_burp}
              onChange={(e) =>
                upd({
          ...
          >
            When enabled, a specialist agent is dispatched immediately alongside every Burp active
            scan, independently investigating the same URL.
          </div>
          ...
          <div className="row spread">
            ...
            <button type="submit" className="btn" disabled={saving}>
              {saving ? "Saving…" : "Save Specialist Settings"}
            </button>
          </div>
        </form>
      )}
