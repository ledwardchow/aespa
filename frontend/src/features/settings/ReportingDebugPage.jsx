...
import { DebugFindingsTable } from "./DebugFindingsTable.jsx";
...
export function ReportingDebugPage() {
  ...
  const replayPromptKey =
    selectedCapture?.kind === "writeup" ? "reporting.writeup" : "reporting.analyse";
  const selectedPromptVersion = promptVersions.find(
    (v) => String(v.id) === String(selectedPromptVersionId),
  );
  ...
  const selectedReplayVersion = replayPromptVersions.find(
    (v) => String(v.id) === String(selectedReplayVersionId),
  );
  ...
  const loadPromptVersions = useCallback(
    async (key, selectedId = "") => {
      const d = await settingsApi.listReportingPromptVersions(key);
  ...
  const loadCaptures = useCallback(async () => {
    const d = await settingsApi.listReportingCaptures();
  ...
  const loadReplays = useCallback(async () => {
    const d = await settingsApi.listReportingReplays();
  ...
  }, [loadPromptVersions, loadReplays, loadCaptures]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      !replayPromptVersions.some((v) => String(v.id) === String(selectedReplayVersionId))
    ) {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(async () => {
      try {
        const next = await settingsApi.getReportingReplay(replay.id);
        setReplay(next);
  ...
  }, [replay?.id, replay?.status, replay, loadReplays]);
  useEffect(() => {
    if (!selectedReplayId) return;
    settingsApi
      .getReportingReplay(selectedReplayId)
      .then(setReplay)
      .catch(() => {});
  }, [selectedReplayId]);
  useEffect(() => {
    ...
    settingsApi
      .getReportingReplay(compareReplayId)
      .then(setCompareReplay)
      .catch(() => {});
  }, [compareReplayId]);
  useEffect(() => {
    ...
    settingsApi
      .getReportingCapture(selectedCaptureId)
      .then(setSelectedCaptureDetail)
      .catch(() => {});
  }, [selectedCaptureId]);
  const savePrompt = async () => {
    ...
    setError(null);
    try {
      const p = await settingsApi.updateReportingPromptVersion(selectedPromptVersion.id, {
  ...
  const createPromptVersion = async () => {
    ...
    setError(null);
    try {
      const p = await settingsApi.createReportingPromptVersion({
        key: promptKey,
        name,
        prompt_text: promptText,
      });
  ...
  const deletePromptVersion = async () => {
    ...
    if (
      !confirm(
        `Delete prompt version "${selectedPromptVersion.name}"? Saved replay findings remain available.`,
      )
    )
      return;
    setPromptSaving(true);
    ...
    setError(null);
    try {
      await settingsApi.deleteReportingPromptVersion(selectedPromptVersion.id);
  ...
  const startReplay = async () => {
    ...
    setReplay(null);
    try {
      const r = await settingsApi.replayReportingCapture(selectedCaptureId, {
    ...
    <>
      ...
      <div className="content scroll-content settings-content">
        <div className="tab-bar settings-tab-bar">
          ...
          <button
            className={"tab-btn" + (tab === "replay" ? " active" : "")}
            onClick={() => {
              setTab("replay");
              loadCaptures().catch(() => {});
            }}
        ...
        {tab === "prompt" && (
          <div className="card">
            ...
            >
              Default versions load from reporting.py. New versions are saved in the Reporting Lab
              database and can be replayed against the same captures.
              <br />
              Reporting Lab uses the DEFAULT LLM setting and does not respect the overriden setting
              in the scan the data came from.
              <br />
              Set the Version name BEFORE clicking new version!
              <br />
              Batch reporting has {"{url}"} and {"{results}"} placeholders.
              <br />
              During-scan writeups have {"{source}"}, {"{base_url}"}, {"{finding_json}"},{" "}
              {"{evidence_json}"}.
            </div>
            ...
            <div className="form-row">
              ...
              >
                {promptKeyVersions.map((v) => (
                  <option key={v.id} value={String(v.id)}>
                    {v.name}
                    {v.is_builtin ? " (from reporting.py)" : ""}
                  </option>
                ))}
              </select>
            </div>
            ...
            {selectedPromptVersion && (
              ...
              >
                <span className="source-badge">
                  {selectedPromptVersion.is_builtin ? "from reporting.py" : "DB version"}
                </span>
                {selectedPromptVersion.updated_at && (
                  <span className="subtle">
                    Updated {fmtDate(selectedPromptVersion.updated_at)}
                  </span>
                )}
              </div>
            )}
            ...
            >
              <button
                className="btn"
                onClick={savePrompt}
                disabled={promptSaving || !promptText.trim() || selectedPromptVersion?.is_builtin}
              >
              ...
              <button
                ...
                disabled={promptSaving || selectedPromptVersion?.is_builtin}
              >
                Delete Version
              </button>
              {promptSaved && (
        ...
        {tab === "replay" && (
          <div className="card">
            ...
            >
              ...
              <span className="mono">{captureDbPath || "reporting debug DB"}</span>.
            </div>
            <div
              className="row"
              style={{
              ...
              >
                ...
                {captures.map((c) => (
                  <option key={c.id} value={String(c.id)}>
                    #{c.id} · {c.kind === "writeup" ? "during-scan writeup" : "final reporting"} ·{" "}
                    {fmtDate(c.created_at)} · {truncUrl(c.url, 52)} · {c.finding_count} findings
                  </option>
                ))}
              </select>
              <select
                ...
                value={selectedReplayVersionId}
                onChange={(e) => setSelectedReplayVersionId(e.target.value)}
              >
                <option value="">Select prompt version…</option>
                {replayPromptVersions.map((v) => (
                  <option key={v.id} value={String(v.id)}>
                    {v.name}
                    {v.is_builtin ? " (default)" : ""}
                  </option>
                ))}
              </select>
              <button
                className="btn secondary"
                onClick={() => loadCaptures().catch((e) => setError(e.message))}
              >
                Refresh
              </button>
              <button
                className="btn"
                onClick={startReplay}
                disabled={replayBusy || !selectedCaptureId || !selectedReplayVersion?.id}
              >
                ...
                >
                  <div>
                    ...
                    <div className="subtle">
                      {selectedCapture.kind === "writeup"
                        ? `Source ${selectedCapture.source || "unknown"}`
                        : `Model ${selectedCapture.llm?.model || "unknown"} · ${selectedCapture.llm?.provider || "unknown"}`}
                    </div>
                    <div className="subtle">
                      Prompt version {selectedReplayVersion?.name || "unknown"}
                    </div>
                  </div>
                  <div>
                    <span className="finding-count-badge">{selectedCapture.finding_count}</span>
                  </div>
                </div>
                {selectedCaptureDetail?.findings?.length > 0 && (
                  <div
                    style={{
                      marginBottom: 12,
                    }}
                  >
                    ...
                    >
                      Original findings in this capture
                    </div>
            ...
            {replay && (
              <>
                ...
                >
                  {["queued", "running"].includes(replay.status) && (
                    <span className="inline-spinner"></span>
                  )}
                  <span className="token-bar-label">{replay.status}</span>
                  <span>{replay.progress_message || ""}</span>
                  {replay.prompt_version_name && (
                    <span className="source-badge">{replay.prompt_version_name}</span>
                  )}
                  {replay.error && (
                    <span
                ...
                {replay.status === "complete" && (
                  ...
                  >
                    <button className="btn" onClick={() => setTab("findings")}>
                      View {replay.finding_count} Debug Finding
                      {replay.finding_count === 1 ? "" : "s"}
                    </button>
                  </div>
                )}
              </>
          ...
          <div className="card">
            ...
            >
              ...
              >
                ...
                {replays.map((r) => (
                  <option key={r.id} value={String(r.id)}>
                    Replay #{r.id} · {r.prompt_version_name || "unknown version"} · {r.status} ·{" "}
                    {fmtDate(r.started_at)} · {r.finding_count} findings
                  </option>
                ))}
              </select>
              <select
              ...
              >
                ...
                {replays
                  .filter((r) => String(r.id) !== String(selectedReplayId))
                  .map((r) => (
                    <option key={r.id} value={String(r.id)}>
                      Replay #{r.id} · {r.prompt_version_name || "unknown version"} · {r.status} ·{" "}
              ...
              <button
                className="btn secondary"
                onClick={() => loadReplays().catch((e) => setError(e.message))}
              >
                Refresh
              </button>
            </div>
              ...
              >
                <div>
                  <div className="form-section-title">
                    Replay #{replay?.id || "—"} · {replay?.prompt_version_name || "unknown version"}
                  </div>
                  {currentFindings.length === 0 ? (
                    ...
                    >
                      No debug findings for this replay.
                    </div>
                  ) : (
                    <DebugFindingsTable findings={currentFindings} />
                  )}
                </div>
                <div>
                  <div className="form-section-title">
                    Replay #{compareReplay.id} ·{" "}
                    {compareReplay.prompt_version_name || "unknown version"}
                  </div>
                  {compareFindings.length === 0 ? (
                    ...
                    <DebugFindingsTable findings={compareFindings} />
                  )}
                </div>
              </div>
            ) : currentFindings.length === 0 ? (
