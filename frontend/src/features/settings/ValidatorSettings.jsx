...
import { validatorPayload, validatorToForm } from "./validatorForm.js";
...
export function ValidatorSettings() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setForm(validatorToForm(await settingsApi.getAdversarialValidatorConfig()));
      } catch (e) {
        setError(e.message);
      }
    })();
  ...
  const onSubmit = async (e) => {
    ...
    setSaved(false);
    try {
      const savedConfig = await settingsApi.upsertAdversarialValidatorConfig(
        validatorPayload(form),
      );
        ...
        <form className="card" onSubmit={onSubmit}>
          ...
          <div className="field">
            ...
            <input
              ...
              min="1"
              max="8"
              value={form.end_scan_max_concurrent}
              disabled={dis}
              onChange={(e) =>
                upd({
            ...
            />
            <div className="field-hint">
              Number of manual or post-scan findings to validate at the same time (1–8). Default: 4.
              Lower this for rate-limited models or fragile targets.
            </div>
          </div>
          ...
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={form.auto_validate_inline}
              disabled={dis}
              onChange={(e) =>
                upd({
          ...
          >
            When enabled, each finding is validated immediately after it is written, while the scan
            is still running. When disabled, validation only runs when triggered manually.
          </div>
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={form.require_concrete_disproof}
              disabled={dis}
              onChange={(e) =>
                upd({
          ...
          >
            When enabled (recommended), the validator must find a specific innocent explanation to
            mark a finding as a false positive — failure to reproduce is not sufficient. When
            disabled, inability to reproduce is treated as a false positive (lenient mode).
          </div>
