...
export function CodeExecutionSettings() {
  ...
  const refresh = async () => {
    const [config, status] = await Promise.all([
      settingsApi.getCodeExecutionConfig(),
      settingsApi.getCodeExecutionStatus(),
    ]);
    setForm(config);
    setRuntime(status);
  };
  ...
  const save = async (event) => {
    ...
    try {
      ...
      await settingsApi.upsertCodeExecutionConfig(payload);
      await refresh();
      setSaved(true);
    } catch (e) {
      setError(e.message);
    ...
    <form className="card" onSubmit={save}>
      <section className="policy-group">
        ...
        <div className="policy-section-copy">
          Let selected agents write short Python programs for unusual parsing, payload generation,
          and bounded workflows. Code runs in a locked-down Docker container with no network. Target
          requests must use the AESPA runtime API and appear in the Traffic Log.
        </div>
        ...
        <label className="toggle-row">
          <input
            type="checkbox"
            checked={form.retain_redacted_source}
            onChange={(e) => update({ retain_redacted_source: e.target.checked })}
          />
      ...
      <section className="policy-group">
        ...
        <div className="policy-section-copy">
          A.L.I.C.E., specialists, and Test Leads are enabled by default when the sandbox feature is
          turned on.
        </div>
        {ROLE_OPTIONS.map(([value, label]) => (
          <label className="toggle-row" key={value}>
            <input
              type="checkbox"
              checked={form.allowed_roles.includes(value)}
              onChange={(e) =>
                update({
      ...
      <section className="policy-group">
        ...
        <div className="two-col">
          <div className="field">
            <label htmlFor="code-image">Docker image</label>
            <input
              id="code-image"
          ...
          <div className="field">
            <label htmlFor="code-timeout">Timeout (seconds)</label>
            <input
              id="code-timeout"
              type="number"
              min="1"
              ...
              onChange={(e) => update({ timeout_s: e.target.value })}
            />
          </div>
          <div className="field">
            <label htmlFor="code-memory">Memory (MiB)</label>
            <input
              id="code-memory"
              type="number"
              min="64"
          ...
          <div className="field">
            <label htmlFor="code-cpu">CPU cores</label>
            <input
              id="code-cpu"
              type="number"
              min="0.25"
          ...
          <div className="field">
            <label htmlFor="code-requests">Requests per execution</label>
            <input
              id="code-requests"
              type="number"
              min="0"
              max="100"
              value={form.max_requests_per_execution}
              onChange={(e) => update({ max_requests_per_execution: e.target.value })}
            />
          </div>
          <div className="field">
            <label htmlFor="code-request-concurrency">Concurrent requests</label>
            <input
              id="code-request-concurrency"
              type="number"
              min="1"
              max="10"
              value={form.max_concurrent_requests}
              onChange={(e) => update({ max_concurrent_requests: e.target.value })}
            />
          </div>
          <div className="field">
            <label htmlFor="code-execution-concurrency">Concurrent executions</label>
            <input
              id="code-execution-concurrency"
              type="number"
              min="1"
              max="8"
              value={form.max_concurrent_executions}
              onChange={(e) => update({ max_concurrent_executions: e.target.value })}
            />
          </div>
          <div className="field">
            <label htmlFor="code-output">Captured output (bytes)</label>
            <input
              id="code-output"
              type="number"
              min="8192"
              max="262144"
              value={form.output_limit_bytes}
              onChange={(e) => update({ output_limit_bytes: e.target.value })}
            />
          </div>
        </div>
      </section>
