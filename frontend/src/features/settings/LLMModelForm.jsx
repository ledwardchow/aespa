...
export function LLMModelForm({ mode, profile, providers, onSaved, onCancel }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const savedProfile =
        mode === "edit"
          ? await settingsApi.updateLLMModel(profile.id, payload)
          : await settingsApi.createLLMModel(payload);
      setSaved(true);
      ...
      <form className="card" onSubmit={onSubmit}>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (!nameTouched || !form.name.trim()) {
                updates.name =
                  provider?.name && newModel ? `${provider.name}/${newModel}` : newModel;
              }
              upd(updates);
            }}
          >
            {providers.map((p) => (
              <option key={p.id} value={p.id}>
                {p.name} ({API_FORMAT_LABELS[p.api_format] || p.api_format})
              </option>
            ))}
          </select>
        </div>
        ...
        <div className="field">
          ...
          >
            ...
            {form.reasoning_effort && !levels.includes(form.reasoning_effort) && (
              <option value={form.reasoning_effort}>{form.reasoning_effort} (saved)</option>
            )}
          </select>
          <div className="field-hint">
            Leave this at Default to keep the provider's current behavior.
          </div>
          {loadingCapabilities && <div className="field-hint">Loading available levels…</div>}
          {capability.strategy === "documented_registry" && (
            <div className="field-hint">Levels from the provider's documented model family.</div>
          )}
          {capability.source === "openrouter" && (
            <div className="field-hint">
              Levels matched from OpenRouter because this provider did not expose them directly.
            </div>
          )}
        </div>
        ...
        <div className="two-col">
          ...
          <div className="field">
            ...
            <label style={{ display: "flex", alignItems: "center", gap: "6px", cursor: "pointer" }}>
              <input
                type="checkbox"
                checked={form.max_context_auto}
                onChange={(e) => upd({ max_context_auto: e.target.checked })}
              />
              <span>
                Auto
            ...
            />
            <div className="field-hint">
              Includes the prompt, tools, conversation, and maximum output.
            </div>
          </div>
          <div className="field">
            <label
            ...
            <input
              type="number"
              required={form.use_temperature}
              disabled={!form.use_temperature}
              min="0"
              max="2"
              step="0.05"
        ...
        <label className="toggle-row">
          <input
            type="checkbox"
            checked={form.force_tool_choice}
            onChange={(e) =>
              upd({
        ...
        >
          ...
          Recommended for standard models to maintain high scanning density. Disable if using custom
          reasoning/thinking models that reject forced tool choice (e.g. DeepSeek-R1,
          deepseek-reasoner).
        </div>
        <div className="divider" />
        <div className="row spread">
          ...
          <div className="row">
            ...
            <button type="submit" className="btn" disabled={saving}>
              {saving ? "Saving…" : mode === "edit" ? "Save model" : "Create model"}
            </button>
          </div>
        </div>
      </form>
