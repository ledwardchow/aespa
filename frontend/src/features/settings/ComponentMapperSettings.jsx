...
export function ComponentMapperSettings() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setForm(toForm(await settingsApi.getComponentMapperConfig()));
      } catch (e) {
        setError(e.message);
      }
    })();
  ...
  const onSubmit = async (e) => {
    ...
    setSaved(false);
    try {
      const updated = await settingsApi.upsertComponentMapperConfig(toPayload(form));
      ...
      {form && (
        <form className="card" onSubmit={onSubmit}>
          ...
          <div className="field-hint" style={{ marginBottom: 16 }}>
            Controls the bounded LLM agent that discovers inbound and outbound interfaces during
            campaign correlation. The selected model is configured in the Component Mapper role
            under LLM Profiles.
          </div>
          ...
          <div className="two-col">
            ...
            <div className="field">
              ...
              />
              <div className="field-hint">Lower this for rate-limited providers. Default: 4.</div>
            </div>
          </div>
          ...
          <div className="two-col">
            ...
            <div className="field">
              ...
              <input
                id="mapper-max-trace-components"
                type="number"
                min="1"
                max="50"
            ...
            <div className="field">
              ...
              <input
                id="mapper-min-trace-confidence"
                type="number"
                min="0"
                max="1"
                step="0.01"
