...
export function CrawlerSettings() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setForm(await settingsApi.getCrawlerConfig());
      } catch (e) {
        setError(e.message);
      }
    })();
  ...
  const onSubmit = async (e) => {
    ...
    setSaved(false);
    try {
      const updated = await settingsApi.upsertCrawlerConfig({
        ...
        llm_max_concurrency:
          form.llm_max_concurrency && form.llm_max_concurrency > 0
            ? form.llm_max_concurrency
            : null,
      });
      setForm(updated);
      setSaved(true);
      ...
      {form && (
        <form className="card" onSubmit={onSubmit}>
          ...
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={!!form.js_endpoint_discovery_enabled}
              onChange={(e) => {
                setSaved(false);
                setForm((f) => ({
                  ...f,
          ...
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={!!form.skip_dangerous_actions}
              onChange={(e) => {
                setSaved(false);
                setForm((f) => ({
                  ...f,
          ...
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={!!form.suppress_form_submit_actions}
              onChange={(e) => {
                setSaved(false);
                setForm((f) => ({
                  ...f,
          ...
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={!!form.block_non_idempotent_interactive_replay}
              onChange={(e) => {
                setSaved(false);
                setForm((f) => ({
                  ...f,
            ...
            />
            <span>
              Block non-idempotent requests (POST/PUT/PATCH/DELETE) during interactive replay
            </span>
          </label>
          <label className="toggle-row">
            <input
              type="checkbox"
              checked={!!form.enable_access_reconciliation}
              onChange={(e) => {
                setSaved(false);
                setForm((f) => ({
                  ...f,
          ...
          <div style={{ marginTop: 16, marginBottom: 8 }}>
            ...
            <div className="subtle" style={{ fontSize: 11, marginBottom: 6 }}>
              Maximum concurrent requests sent to the LLM during page analysis. Default is unlimited
              (0 or blank). Excess requests will queue automatically.
            </div>
            <input
              type="number"
              min="0"
