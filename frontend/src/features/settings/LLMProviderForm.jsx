...
import { PROVIDER_MODEL_PLACEHOLDERS, PROVIDER_BASE_URL_PLACEHOLDERS } from "./providerMetadata.js";
...
function CodexConnectionCard() {
  ...
  const refresh = useCallback(async () => {
    try {
      const next = await settingsApi.getCodexStatus();
      setState(next);
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      await settingsApi.saveCodexConfig({ executable_path: path.trim() || null });
      await refresh();
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  const login = async () => {
    ...
    let pending = false;
    try {
      const result = await settingsApi.startCodexLogin();
      if (result.verificationUrl && result.userCode) {
        setLoginChallenge(result);
        pending = true;
      } else {
        throw new Error(
          "Codex did not return a device-code login. Upgrade Codex CLI and try again.",
        );
      }
    } catch (e) {
      setError(e.message);
  ...
  const cancelLogin = async () => {
    ...
    setError(null);
    try {
      await settingsApi.cancelCodexLogin({ loginId: loginChallenge.loginId });
      setLoginChallenge(null);
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  return (
    <div className="form-section" style={{ marginTop: 12 }}>
      ...
      <div className="field-hint">
        Install Codex separately. AESPA uses the default account already signed in through the local
        Codex CLI and never copies those credentials into its database.
      </div>
      <div className="field">
        <label>
          Codex executable path <span className="field-optional">(optional)</span>
        </label>
        <input
          value={path}
          placeholder="Leave blank to use codex from PATH"
          onChange={(e) => setPath(e.target.value)}
        />
      </div>
      <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
        <button type="button" className="btn secondary sm" disabled={busy} onClick={save}>
          Save path
        </button>
        <button
          type="button"
        ...
        {state?.account && (
          <button type="button" className="btn ghost sm" disabled={busy} onClick={logout}>
            Sign out
          </button>
        )}
      ...
      {loginChallenge && (
        <div className="form-section" style={{ marginTop: 12 }}>
          ...
          <div className="field-hint">
            Open the OpenAI verification page, enter this one-time code, and return to AESPA. This
            page checks the login automatically.
          </div>
          <div className="row" style={{ gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <code style={{ fontSize: 18, letterSpacing: 1 }}>{loginChallenge.userCode}</code>
            <a
              className="btn secondary sm"
              href={loginChallenge.verificationUrl}
              target="_blank"
              rel="noreferrer"
            >
              Open verification page
            </a>
            <button type="button" className="btn ghost sm" onClick={cancelLogin}>
              Cancel
            </button>
          </div>
        </div>
      ...
      {state && (
        <div className="field-hint" style={{ marginTop: 8 }}>
          {state.installed
            ? `Detected ${state.version || "Codex"} at ${state.detected_executable || "configured path"}.`
            : "Codex CLI was not found."}
          {state.account
            ? ` Signed in${state.account.planType ? ` with ${state.account.planType}` : ""}.`
            : " Not signed in."}
        </div>
      )}
      {error && (
...
function CopilotConnectionCard({ value, onSelect }) {
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await settingsApi.listCopilotAccounts();
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const result = await settingsApi.getCopilotLogin(loginChallenge.login_id);
        if (stopped) return;
        ...
        if (result.status === "failed" || result.status === "cancelled") {
          ...
          if (result.status === "failed")
            setError(result.error || "GitHub Copilot sign-in failed.");
          return;
        }
  ...
  const login = async () => {
    ...
    setMessage(null);
    try {
      const result = await settingsApi.startCopilotLogin();
  ...
  const cancelLogin = async () => {
    ...
    setError(null);
    try {
      await settingsApi.cancelCopilotLogin(loginChallenge.login_id);
      setLoginChallenge(null);
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  const selectedMissing =
    value && !accounts.some((account) => account.login.toLowerCase() === value.toLowerCase());

  return (
    <div className="form-section" style={{ marginTop: 12 }}>
      <div className="form-section-title">Copilot connection</div>
      <div className="field-hint">
        Accounts are authenticated and stored by Copilot CLI. AESPA only saves the selected GitHub
        username.
      </div>
      <div className="field">
        <label htmlFor="copilot-account-selector">Copilot account</label>
        <select
          id="copilot-account-selector"
          className="select"
          value={value}
        ...
        >
          <option value="">
            Copilot CLI default{defaultAccount ? ` — ${defaultAccount.login}` : ""}
          </option>
      ...
      <div className="row" style={{ gap: 8, flexWrap: "wrap" }}>
        <button type="button" className="btn secondary sm" disabled={busy} onClick={login}>
          Sign in another account
        </button>
        <button type="button" className="btn ghost sm" disabled={busy || loading} onClick={refresh}>
          Refresh accounts
        </button>
      </div>
      {loginChallenge && (
        <div className="form-section" style={{ marginTop: 12 }}>
          <div className="form-section-title">Finish signing in</div>
          <div className="field-hint">
            Open GitHub, enter this one-time code, and authorize GitHub Copilot CLI. AESPA will
            detect completion automatically.
          </div>
          <div className="row" style={{ gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            <code style={{ fontSize: 18, letterSpacing: 1 }}>{loginChallenge.user_code}</code>
            <button type="button" className="btn ghost sm" onClick={copyCode}>
              Copy code
            </button>
            <a
              className="btn secondary sm"
      ...
      {!loading && accounts.length === 0 && !loginChallenge && (
        <div className="field-hint" style={{ marginTop: 8 }}>
          No saved Copilot CLI accounts were found. Sign in to add one.
        </div>
      )}
      {message && (
...
export function LLMProviderForm({ mode, provider, onSaved, onCancel }) {
  ...
  const selectCopilotUsername = useCallback(
    (username) => {
      upd({ username });
    },
    [upd],
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const savedProvider =
        mode === "edit"
          ? await settingsApi.updateLLMProvider(provider.id, payload)
          : await settingsApi.createLLMProvider(payload);
      setSaved(true);
      ...
      <form className="card" onSubmit={onSubmit}>
        ...
        {isBedrockProvider(form.api_format) && (
          <div className="field">
            ...
            <select
              className="select"
              value={form.region || BEDROCK_DEFAULT_REGIONS[form.api_format]}
              onChange={(e) =>
                upd({
            ...
            >
              {!BEDROCK_REGIONS[form.api_format].some(([region]) => region === form.region) &&
                form.region && (
                  <option value={form.region}>{form.region} (existing configuration)</option>
                )}
              {BEDROCK_REGIONS[form.api_format].map(([region, name]) => (
            ...
            <div className="field-hint">
              AESPA builds the fixed{" "}
              {form.api_format === "bedrock_mantle" ? "Bedrock Mantle" : "Bedrock Runtime"} endpoint
              for this region.
            </div>
          </div>
        )}
        {!["factory_droid", "openai_codex", "google_antigravity"].includes(form.api_format) &&
          !isBedrockProvider(form.api_format) && (
            <div className="field">
              <label>
                Base URL <span className="field-optional">(optional)</span>
              </label>
              <input
                type="url"
                value={form.base_url}
                placeholder={PROVIDER_BASE_URL_PLACEHOLDERS[form.api_format] || ""}
                onChange={(e) =>
                  upd({
              ...
              {form.api_format === "github_copilot" && (
                <div className="field-hint">
                  No base URL is needed. AESPA uses the official GitHub Copilot SDK.
                </div>
              )}
            </div>
          )}
        {form.api_format === "factory_droid" && (
          <div className="field-hint">
        ...
        {form.api_format === "openai_codex" && (
          <>
            <div className="field-hint">
              Uses the local Codex CLI's default ChatGPT sign-in through the installed Codex
              app-server. No API key or base URL is used.
            </div>
            <CodexConnectionCard />
          </>
        )}
        ...
        {!["factory_droid", "openai_codex", "google_antigravity"].includes(form.api_format) && (
          <div className="field">
            ...
            {form.api_format === "bedrock" && (
              <div className="field-hint">
                When blank, Aespa uses boto3 credentials from AWS_PROFILE, environment variables,
                SSO, or the instance/task role.
              </div>
            )}
            {form.api_format === "bedrock_mantle" && (
              <div className="field-hint">
            ...
            {form.api_format === "github_copilot" && (
              <div className="field-hint">
                Leave blank to use the Copilot username above, or Copilot CLI's selected default
                account when no username is set. An explicit token takes precedence. For headless
                use, enter a GitHub user token whose account has Copilot access or set
