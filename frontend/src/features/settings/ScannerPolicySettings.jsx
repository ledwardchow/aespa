...
import { policyToForm, policyPayload, SCAN_MODE_OPTIONS } from "../../shared/runs/policy.js";
    ...
    <>
      <section className="policy-group">
        ...
        <div className="policy-section-copy">
          Choose how much active testing to perform. The selected mode determines which HTTP method
          list the scan uses.
        </div>
        <div className="scan-mode-picker">
          ...
          <div className="scan-mode-summary" aria-live="polite">
            ...
            <strong>
              {SCAN_MODE_OPTIONS.find(([value]) => value === form.scan_mode)?.[1] || form.scan_mode}
            </strong>
        ...
        <div className="policy-subsection">
          ...
          <div className="field-hint">
            Enter comma-separated methods, for example GET, POST. The highlighted list is active;
            the other lists are saved for when you switch modes.
          </div>
          <div className="scan-method-grid">
            {SCAN_MODE_OPTIONS.map(([value, label]) => {
              ...
              return (
                <div className={"scan-method-card" + (selected ? " active" : "")} key={value}>
      ...
      <section className="policy-group">
        ...
        <div className="policy-section-copy">
          These limits apply to the scan as a whole. They are independent of the selected mode and
          its HTTP methods.
        </div>
        <div className="two-col">
          <div className="field">
            <label htmlFor="global-max-probes">Maximum probes per page</label>
            <input
              id="global-max-probes"
              type="number"
              min="0"
              max="500"
              value={form.max_probes_per_page}
              onChange={(e) =>
                upd({
          ...
          <div className="field">
            <label htmlFor="global-request-timeout">Request timeout (seconds)</label>
            <input
              id="global-request-timeout"
              type="number"
              min="1"
              max="120"
              step="0.5"
          ...
          <div className="field">
            <label htmlFor="global-min-delay">Minimum delay between requests (seconds)</label>
            <input
              id="global-min-delay"
              type="number"
              min="0"
          ...
          <div className="field">
            <label htmlFor="global-max-request-body">Maximum request body size (bytes)</label>
            <input
              id="global-max-request-body"
              type="number"
              min="0"
              max={10 * 1024 * 1024}
              value={form.max_request_body_bytes}
              onChange={(e) =>
                upd({
        ...
        <div className="field">
          <label htmlFor="global-response-body-limit">Maximum response data read (bytes)</label>
          <input
            id="global-response-body-limit"
            type="number"
            min="1024"
            max={10 * 1024 * 1024}
            value={form.response_body_read_limit_bytes}
            onChange={(e) =>
              upd({
      ...
      <section className="policy-group">
        ...
        <div className="two-col">
          <div className="field">
            <label htmlFor="global-allowed-schemes">Allowed URL schemes</label>
            <input
              id="global-allowed-schemes"
              type="text"
              value={form.allowed_schemes}
              onChange={(e) =>
                upd({
          ...
          <div className="field">
            <label htmlFor="global-blocked-headers">Blocked request headers</label>
            <input
              id="global-blocked-headers"
              type="text"
            ...
            />
            <div className="field-hint">
              Comma-separated header names, for example: host, cookie
            </div>
          </div>
        </div>
...
function GlobalHttpHeadersSettings() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const config = await settingsApi.getGlobalHttpHeader();
        setHdrCfg(config);
  ...
  const saveHeader = async (e) => {
    ...
    setError(null);
    try {
      const updated = await settingsApi.upsertGlobalHttpHeader({
        headers: hdrForm
  ...
  return (
    <section className="card global-http-headers-card">
      <div className="form-section-title">Global Extra HTTP Headers</div>
      <div className="field-hint" style={{ marginBottom: 12 }}>
        Add headers to every request made by the scanner and crawler (Playwright and HTTPX). These
        do not affect requests sent to LLMs.
      </div>
          ...
          <div className="table-wrap global-header-table-wrap">
            <table className="global-header-table">
              ...
              <tbody>
                {hdrForm.length === 0 && (
                  <tr>
                    <td colSpan="3" className="subtle global-header-empty">
                      No headers configured.
                    </td>
                  </tr>
                )}
                {hdrForm.map((header, index) => (
                  <tr key={index}>
                    <td>
                      <input
                        ...
                        onInput={(e) => {
                          ...
                          setHdrForm((headers) =>
                            headers.map((item, itemIndex) =>
                              itemIndex === index ? { ...item, header_name: e.target.value } : item,
                            ),
                          );
                        }}
                      />
                      ...
                      <input
                        ...
                        onInput={(e) => {
                          ...
                          setHdrForm((headers) =>
                            headers.map((item, itemIndex) =>
                              itemIndex === index
                                ? { ...item, header_value: e.target.value }
                                : item,
                            ),
                          );
                        }}
...
function PolicySettings({ Fields }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        setForm(policyToForm(await settingsApi.getScannerPolicy()));
      } catch (e) {
        setError(e.message);
      }
    })();
  ...
  const onSubmit = async (e) => {
    ...
    setSaved(false);
    try {
      const savedPolicy = await settingsApi.upsertScannerPolicy(policyPayload(form));
...
export function GlobalPolicySettings({ tab = "scan-behaviour" }) {
  return (
    <>
      {tab === "scan-behaviour" && (
        <div
          id="global-scan-behaviour-panel"
          role="tabpanel"
          aria-labelledby="global-scan-behaviour-tab"
        >
          <PolicySettings Fields={GlobalPolicyFields} />
        </div>
      )}
      {tab === "headers" && (
        <div id="global-headers-panel" role="tabpanel" aria-labelledby="global-headers-tab">
