...
export function useComponents(applicationId, onChanged) {
  ...
  const load = useCallback(async () => {
    try {
      setComponents(await applicationsApi.listAppComponents(applicationId));
    } catch (e) {
      setError(e.message);
    }
  }, [applicationId]);
  ...
  const loadSnapshotHistory = useCallback(
    async (componentId) => {
      try {
        const snapshots = await applicationsApi.listComponentSnapshots(applicationId, componentId);
  ...
  const createComponent = useCallback(
    async (body) => {
      await applicationsApi.createAppComponent(applicationId, body);
      await load();
      onChanged?.();
    },
  ...
  const updateComponent = useCallback(
    async (componentId, body) => {
      await applicationsApi.updateAppComponent(applicationId, componentId, body);
      await load();
      onChanged?.();
    },
    [applicationId, load, onChanged],
  ...
  const deleteComponent = useCallback(
    async (componentId) => {
      await applicationsApi.deleteAppComponent(applicationId, componentId);
      await load();
      onChanged?.();
    },
    [applicationId, load, onChanged],
  ...
  const uploadSnapshot = useCallback(
    async (componentId, file) => {
      await applicationsApi.uploadComponentSnapshot(applicationId, componentId, file);
      await load();
  ...
  const deleteSnapshot = useCallback(
    async (componentId, snapshotId) => {
      await applicationsApi.deleteComponentSnapshot(applicationId, componentId, snapshotId);
      await load();
      await loadSnapshotHistory(componentId);
      onChanged?.();
    },
