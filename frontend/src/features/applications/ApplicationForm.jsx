...
export function ApplicationForm({ applicationId }) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const app = editing
        ? await applicationsApi.updateApplication(applicationId, body)
        ...
        <form className="card" style={{ maxWidth: 560 }} onSubmit={onSubmit}>
          ...
          <div className="field">
            ...
            <textarea
              ...
              placeholder="What this application is and how its parts fit together."
            />
          </div>
          <div className="row spread" style={{ marginTop: 16 }}>
            <button
              type="button"
              className="btn ghost"
              onClick={() => nav(editing ? `#/applications/${applicationId}` : "#/applications")}
            >
              Cancel
            </button>
            <button type="submit" className="btn" disabled={saving || !name.trim()}>
              {saving ? "Saving…" : editing ? "Save" : "Create application"}
            </button>
          </div>
        </form>
      </div>
