...
export function ApiCollectionForm({ collectionId }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const d = await apiCollectionsApi.getApiCollection(collectionId);
        setForm({
          name: d.name,
          base_url: d.base_url,
  ...
  const onSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await apiCollectionsApi.updateApiCollection(collectionId, payload);
        nav(`#/apis/${collectionId}`);
      } else {
        const c = await apiCollectionsApi.createApiCollection(payload);
        nav(`#/apis/${c.id}`);
      }
    } catch (e) {
      setError(e.message);
  ...
  const bc = isEdit ? (
    <>
      <Crumb href={`#/apis/${collectionId}`}>{form.name || "API collection"}</Crumb>
      <Sep />
      Edit
    </>
  ) : (
          ...
          <form className="card" onSubmit={onSubmit}>
            ...
            <div className="field">
              ...
              <textarea
                ...
                placeholder="What these APIs do, scope, contacts…"
                onChange={(e) =>
                  upd({
            ...
            <div className="row spread">
              <button
                type="button"
                className="btn ghost"
                onClick={() => (isEdit ? nav(`#/apis/${collectionId}`) : nav("#/apis"))}
              >
                Cancel
              </button>
