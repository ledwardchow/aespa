...
export function ComponentsTab({ systemId, onChanged }) {
  const {
    components,
    error,
    setError,
    ...
    uploadSnapshot,
    deleteSnapshot,
  } = useComponents(systemId, onChanged);
  ...
  return (
    <div>
      ...
      <div className="row spread" style={{ marginBottom: 14 }}>
        ...
        <button className="btn secondary sm" onClick={() => setCreating((c) => !c)}>
      ...
      {creating && (
        <NewComponentForm
          ...
          onCreate={async (body) => {
            await createComponent(body).catch((e) => setError(e.message));
            setCreating(false);
          }}
        />
      )}
      {components === null && <div className="subtle">Loading…</div>}
      {components !== null && components.length === 0 && !creating && (
        <EmptyState
          title="No code components yet"
          sub="Add a component for each repository or micro-frontend that makes up this system, then upload its ZIP snapshot."
          action={
      ...
      {components && components.length > 0 && (
        <div className="app-component-grid">
          {components.map((c) => (
            <ComponentCard
              key={c.id}
              component={c}
              ...
              onDelete={() => {
                if (
                  !confirm(
                    `Delete component "${c.name}"? This removes all of its snapshots and cannot be undone.`,
                  )
                )
                  return;
                deleteComponent(c.id).catch((e) =>
                  setError(
                    e.status === 409
                      ? `Cannot delete "${c.name}" — it is still referenced by a campaign.`
                      : e.message,
                  ),
                );
              }}
              ...
              onDeleteSnapshot={(snapshotId) =>
                deleteSnapshot(c.id, snapshotId).catch((e) =>
                  setError(
                    e.status === 409
                      ? "Cannot delete this snapshot — a campaign still references it."
                      : e.message,
                  ),
                )
              }
...
function NewComponentForm({ onCancel, onCreate }) {
  ...
  return (
    <form className="card" style={{ marginBottom: 14, maxWidth: 480 }} onSubmit={submit}>
      <div className="field">
        <label>
          Component name <span className="field-required">*</span>
        </label>
        <input
          type="text"
          value={name}
          onChange={(e) => setName(e.target.value)}
          placeholder="e.g. checkout-ui"
          autoFocus
        />
      </div>
      <div className="field">
        <label>
          Role <span className="subtle">(optional)</span>
        </label>
        <input
          type="text"
          value={role}
          onChange={(e) => setRole(e.target.value)}
          placeholder="e.g. Micro-frontend, Backend API"
        />
      </div>
      <div className="field">
        <label>
...
function ComponentCard({
  ...
  return (
    <div className="card app-component-card">
      ...
      {editing ? (
        <div className="field" style={{ marginTop: 8 }}>
          ...
          <div className="row spread" style={{ marginTop: 6 }}>
            <button className="btn danger-outline sm" onClick={onDelete}>
              Delete component
            </button>
        ...
        <>
            ...
            {latest ? (
              <>
                ...
                <div className="subtle" style={{ fontSize: 11 }}>
                  SHA {shortHash(latest.sha256)} · {formatBytes(latest.size_bytes)} ·{" "}
                  {fmtDate(latest.created_at)} · latest
                </div>
              </>
            ) : (
              <div className="subtle" style={{ fontSize: 12 }}>
          ...
          {historyOpen && (
            <div className="app-snapshot-history">
              {snapshots === undefined && (
                <div className="subtle" style={{ fontSize: 12 }}>
                  Loading…
                </div>
              ...
              {snapshots &&
                snapshots.map((s) => (
                  <div key={s.id} className="app-snapshot-row">
                    <div>
                      <div className="mono" style={{ fontSize: 12 }}>
                        {s.filename}
                      </div>
                      <div className="subtle" style={{ fontSize: 11 }}>
                        SHA {shortHash(s.sha256)} · {formatBytes(s.size_bytes)} ·{" "}
