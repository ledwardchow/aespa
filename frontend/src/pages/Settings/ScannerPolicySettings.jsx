...
function GlobalPolicyFields({ form, upd }) {
  return <>
    ...
    <div className="field">
      ...
      <select className="select" value={form.scan_mode} onChange={e => upd({
    ...
    <div className="two-col">
      <div className="field"><label>Max probes per page</label>
        <input type="number" min="0" max="500" value={form.max_probes_per_page} onChange={e => upd({
          max_probes_per_page: e.target.value
        })} /></div>
      <div className="field"><label>Request timeout (seconds)</label>
        <input type="number" min="1" max="120" step="0.5" value={form.request_timeout_s} onChange={e => upd({
          request_timeout_s: e.target.value
        })} /></div>
      <div className="field"><label>Minimum delay (seconds)</label>
        <input type="number" min="0" max="60" step="0.05" value={form.min_delay_s} onChange={e => upd({
          min_delay_s: e.target.value
        })} /></div>
      <div className="field"><label>Max request body bytes</label>
        <input type="number" min="0" max={10 * 1024 * 1024} value={form.max_request_body_bytes} onChange={e => upd({
          max_request_body_bytes: e.target.value
        })} /></div>
    </div>
    <div className="field"><label>Response body read limit bytes</label>
      <input type="number" min="1024" max={10 * 1024 * 1024} value={form.response_body_read_limit_bytes} onChange={e => upd({
    ...
    <div className="two-col">
      <div className="field"><label>Allowed schemes</label>
        <input type="text" value={form.allowed_schemes} onChange={e => upd({
          allowed_schemes: e.target.value
        })} /></div>
      <div className="field"><label>Blocked headers</label>
        <input type="text" value={form.blocked_headers} onChange={e => upd({
    ...
    <div className="two-col">
      <div className="field"><label>Passive</label>
        <input type="text" value={form.methods_passive} onChange={e => upd({
          methods_passive: e.target.value
        })} /></div>
      <div className="field"><label>Safe active</label>
        <input type="text" value={form.methods_safe_active} onChange={e => upd({
          methods_safe_active: e.target.value
        })} /></div>
      <div className="field"><label>Aggressive</label>
        <input type="text" value={form.methods_aggressive} onChange={e => upd({
          methods_aggressive: e.target.value
        })} /></div>
      <div className="field"><label>Destructive</label>
        <input type="text" value={form.methods_destructive} onChange={e => upd({
