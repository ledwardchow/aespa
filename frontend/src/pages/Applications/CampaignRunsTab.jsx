...
import { campaignDisplayStatus, campaignMemberDisplayStatus, safeParseJson } from "./_helpers";
...
export function CampaignRunsTab({ applicationId, campaign, error, resumeSource, resumeTarget, busy }) {
  ...
  const canResumeMember = !["sast_running", "correlating", "dast_running"].includes(campaignDisplayStatus(campaign));
        ...
        <table>
          ...
          <tbody>
            {sourceRuns.map(m => <tr key={m.id}>
              ...
              <td><StatusBadge status={campaignMemberDisplayStatus(m)} /></td>
              <td>
                {m.sast_run_id && <a className="btn secondary sm" href={`#/sast-runs/${m.sast_run_id}/progress`}>Open →</a>}
                {canResumeMember && resumable(campaignMemberDisplayStatus(m)) && <button className="btn secondary sm" title="Resume only this SAST action" disabled={busy} onClick={() => resumeSource(m.id)}>Resume SAST</button>}
              </td>
            </tr>)}
          ...
          <tbody>
            {targetRuns.map(m => <tr key={m.id}>
              ...
              <td>
                ...
                {canResumeMember && resumable(campaignMemberDisplayStatus(m)) && <button className="btn secondary sm" title={`Resume only this ${m.target_type === "site" ? "web" : "API"} action`} disabled={busy} onClick={() => resumeTarget(m.id)}>Resume {m.target_type === "site" ? "web" : "API"}</button>}
              </td>
            </tr>)}
