...
export function CampaignActivityTab({ applicationId, campaignId, campaign }) {
  ...
  useEffect(() => {
    ...
    const url = api.getCampaignActivityStreamUrl(applicationId, campaignId, lastEventIdRef.current);
    ...
    es.onmessage = e => {
      ...
      } catch { /* heartbeat comment lines never reach onmessage */ }
    };
    ...
    const initialTimer = setTimeout(() => { if (!cancelled) setLoading(false); }, INITIAL_LOAD_TIMEOUT_MS);
  ...
  return <div>
    <div className="row spread" style={{ marginBottom: 10 }}>
      <span className="subtle">{entries.length} event{entries.length !== 1 ? "s" : ""}</span>
      {!streaming && <span className="badge warning">Live stream unavailable — showing status-poll fallback</span>}
    </div>
    {warnings.length > 0 && <div className="alert warning" style={{ marginBottom: 12 }}>
      {warnings.map((w, i) => <div key={i}>{w}</div>)}
    </div>}
    {loading ? <div className="subtle" style={{ padding: 24, textAlign: "center" }}>Loading…</div> : entries.length === 0 ? <div className="subtle" style={{ padding: 24, textAlign: "center" }}>
      No activity recorded yet. Activity appears here as the campaign runs, and reloading this page replays everything recorded so far.
    </div> : <div className="activity-feed">
      {entries.map(e => <div key={e.id} className="activity-entry">
        <span className="activity-ts">{new Date(e.ts).toLocaleTimeString("en-US", { hour12: false })}</span>
        <span className="activity-badge phase-other">{(e.status || e.type || "").toUpperCase()}</span>
        <span className="activity-msg">{e.task || e.message || ""}{e.outcome ? ` → ${e.outcome}` : ""}</span>
      </div>)}
    </div>}
  </div>;
}
