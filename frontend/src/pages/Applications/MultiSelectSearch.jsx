...
export function MultiSelectSearch({ items, selectedIds, onToggle, placeholder = "Search…", emptyLabel = "Nothing to select.", renderMeta }) {
  ...
  return <div className="multi-select">
    ...
    <div className="multi-select-list">
      {filtered.length === 0 && <div className="subtle" style={{ padding: "10px 4px" }}>{emptyLabel}</div>}
      {filtered.map(it => {
        const checked = selectedIds.has(it.id);
        return <label key={it.id} className={"multi-select-row" + (checked ? " checked" : "")}>
