...
import { IconExternalLink, IconSend } from "../../components/Icons";
import {
...
export function AliceChatPanel({
  ...
  return <div className={"alice-chat-container" + (popout ? " alice-chat-container--popout" : "")} onClick={e => e.stopPropagation()}>
    <div className="alice-chat-tabs-bar">
      ...
      <button className="alice-chat-add-tab-btn" onClick={createAliceTab} title="New Session" aria-label="New Session">
        +
      </button>
      {!popout && <button className="alice-chat-popout-btn" onClick={handlePopOut} title="Open A.L.I.C.E. in a new window or tab" aria-label="Open A.L.I.C.E. in a new window or tab">
        <IconExternalLink />
      </button>}
    </div>
    <div
    ...
    >
      {aliceMessages.map((msg, _msgIdx) => {
        ...
        if (msg.type === "thinking") {
          ...
          return <React.Fragment key={msg.id}>{segments.map((segment, segmentIndex) => {
            if (segment.kind === "message") {
              return <div key={msg.id + ":m" + segmentIndex} className="alice-msg-row alice-msg-row--alice">
            ...
            return renderAliceTraceBox(segmentKey, segment.text, msg.stepData || {}, aliceExpandedThinkIds.has(segmentKey), () => setAliceExpandedThinkIds(prev => {
      ...
      {isActiveThinking && <div className="alice-msg-row alice-msg-row--alice">
    ...
    <div className="alice-chat-input-bar">
      ...
      </button> : <button className="alice-chat-input-btn" disabled={!aliceInputText.trim()} onClick={handleAliceSend} title="Send Instruction" aria-label="Send Instruction">
        <IconSend />
      </button>}
    </div>
  </div>;
