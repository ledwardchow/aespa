...
export function useSitemapGraph({
  ...
  useEffect(() => {
    ...
    if (structureKey === previousStructureKeyRef.current && simulationRef.current) {
      ...
      d3.select(svgRef.current).selectAll("circle.node-dot").filter(node => node && node.id != null).attr("fill", nodeColor).attr("stroke", node => node.status === "failed" ? "#fbbf24" : "var(--bg)");
      d3.select(svgRef.current).selectAll("g.node-group").each(function(nodeData) {
        if (!nodeData) return;
    ...
    const simulation = d3.forceSimulation(nodes).force("link", d3.forceLink(links).id(node => node.id).distance(110).strength(0.8)).force("charge", d3.forceManyBody().strength(-350)).force("center", d3.forceCenter(width / 2, height / 2)).force("x", d3.forceX(width / 2).strength(gravityRef.current)).force("y", d3.forceY(height / 2).strength(gravityRef.current)).force("collision", d3.forceCollide(22));
    const node = graphGroup.append("g").selectAll("g").data(nodes).join("g").attr("class", "node-group").attr("cursor", "pointer").call(d3.drag().on("start", (event, draggedNode) => {
    ...
    node.each(function(nodeData) {
      if (!nodeData) return;
      if (needsLlmAnalysis(nodeData)) {
        d3.select(this).insert("circle", ":first-child").attr("class", "node-llm-pulse").attr("r", 10);
      }
    });
    ...
    node.append("title").text(node => {
      const err = node.status === "failed" && node.error_message ? `\n(Failed: ${node.error_message})` : node.status === "failed" ? "\n(Failed)" : "";
  ...
  useEffect(() => {
    ...
    if (currentUrl) {
      ...
      svg.select("g").selectAll("g.node-group").filter(node => node && node.url && node.url.replace(/\/$/, "") === normalizedUrl).insert("circle", ":first-child").attr("class", "node-crawl-pulse").attr("r", 10);
    }
