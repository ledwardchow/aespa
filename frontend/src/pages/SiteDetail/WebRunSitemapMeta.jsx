...
export function WebRunSitemapMeta({ run, graph, crawlUsername, profiles, onRunUpdate, onError, crawlCredentialId, onCrawlCredentialChange }) {
  ...
  const queuedCount = pendingNodes.filter(n => n.analysis_status === "queued" || n.analysis_status === "pending" || !n.analysis_status).length;
  ...
  return <div className="run-meta">
    ...
    {editing ? <div className="run-stat-edit">
      ...
      <div className="run-stat"><span className="run-stat-val" style={{ fontSize: 12 }}>{run.llm_max_concurrency ? `${run.llm_max_concurrency} max` : "Unlimited"}</span><span className="run-stat-lbl">LLM Concurrency</span></div>
      ...
      {run.status !== "running" && <button className="btn ghost sm" style={{ alignSelf: "center", marginLeft: 4 }} title="Edit depth / pages / LLM concurrency" onClick={edit}>✎</button>}
    </>}
    {!multiUser && <>
    ...
    {(run.status === "running" || (run.llm_pending || 0) > 0 || (run.llm_completed || 0) > 0 || pendingNodes.length > 0) && (
      <div
        className="run-stat"
            ...
            <div className="llm-queue-popover-body">
                ...
                <>
                  ...
                  {pendingNodes.map(node => (
                    <div key={node.id} className="llm-queue-item">
                      ...
                      <span className={`pill ${node.analysis_status === "analyzing" ? "warning" : "neutral"} sm`}>
              ...
              {skippedNodes.length > 0 && (
                <>
                  ...
                  {skippedNodes.slice(-10).reverse().map(node => (
                    <div key={node.id} className="llm-queue-item">
                      <span className="llm-queue-item-url" title={node.url}>
                        {truncUrl(node.url, 38)}
                      </span>
              ...
              {completedNodes.length > 0 && (
                <>
                  ...
                  {completedNodes.slice(-10).reverse().map(node => (
                    <div key={node.id} className="llm-queue-item">
                      <span className="llm-queue-item-url" title={node.url}>
              ...
              {pendingNodes.length === 0 && completedNodes.length === 0 && skippedNodes.length === 0 && (
                <div className="subtle" style={{ padding: "12px 0", textAlign: "center", fontSize: 11 }}>
