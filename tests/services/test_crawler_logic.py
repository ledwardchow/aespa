...
@pytest.mark.parametrize(
    ("url", "text", "expected"),
    [
        ("https://target.local/logout", None, True),
        ("https://target.local/account/log-out", None, True),
        ("https://target.local/session/end", "Sign out", True),
        ("https://target.local/profile", "Log out", True),
        ("https://target.local/account", None, False),
        ("https://target.local/signature", "Sign document", False),
    ],
)
def test_session_ending_url_detection(url, text, expected):
    assert crawler._is_session_ending_url(url, text) is expected
...
@pytest.mark.parametrize(
    ("cred", "expected"),
    [
        (_CredWithLogin(), "https://target.local/admin/login"),
        (object(), "https://target.local/login"),
    ],
)
def test_login_url_for_credential(cred, expected):
    assert (
        crawler._login_url_for_credential("https://target.local/login", cred)
        == expected
    )
