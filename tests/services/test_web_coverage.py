...
def test_web_coverage_seed_creates_cells(db_engine, db_session, run):
...
def test_web_coverage_seed_idempotent(db_engine, db_session, run):
...
def test_web_coverage_update_cell_creates(db_engine, db_session, run):
...
def test_web_coverage_update_cell_updates_status(db_engine, db_session, run):
    page = _make_page(db_session, run, "http://example.com/api", ["A01"])
    seed_web_workprogram(run.id)
...
def test_web_coverage_update_cell_no_downgrade(db_engine, db_session, run):
    page = _make_page(db_session, run, "http://example.com/api", ["A01"])
...
def test_web_coverage_update_cell_appends_finding(db_engine, db_session, run):
