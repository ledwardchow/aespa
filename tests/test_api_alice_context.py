...
def test_context_tool_endpoint_list_and_detail(db_session):
    ...
    try:
        ...
        lst_post = _run_api_context_tool(
            coll.id, run.id, "endpoint_list", {"method": "POST"}
        )
        assert lst_post["count"] == 0

        # endpoint_list search
        lst_search = _run_api_context_tool(
            coll.id, run.id, "endpoint_list", {"search": "product"}
        )
        assert lst_search["count"] == 1

        # endpoint_detail by id
        detail = _run_api_context_tool(
            coll.id, run.id, "endpoint_detail", {"endpoint_id": ep.id}
        )
        ...
        miss = _run_api_context_tool(
            coll.id, run.id, "endpoint_detail", {"endpoint_id": 9999}
        )
...
def test_set_coverage_marks_cell_and_matrix_reflects_it(db_session):
    ...
    try:
        ...
        before = _run_api_context_tool(
            coll.id, run.id, "coverage_matrix", {"endpoint_id": ep.id}
        )
        api1 = next(c for c in before["cells"] if c["owasp_api_category"] == "API1")
        assert api1["status"] == "not_started"

        # Mark it covered.
        res = _run_api_context_tool(
            coll.id,
            run.id,
            "set_coverage",
            {
...
def test_report_finding_auto_links_coverage_cell(db_session):
    ...
    try:
        ...
        assert res["ok"] is True
        assert res["coverage_cell"] == {
            "endpoint_id": ep.id,
            "owasp_api_category": "API1",
        }

        matrix = _run_api_context_tool(
            coll.id,
            run.id,
            "coverage_matrix",
            {
...
def test_set_coverage_validates_inputs(db_session):
    ...
    db_mod._engine = db_session.get_bind()
    try:
        bad_status = _run_api_context_tool(
            coll.id,
            run.id,
            "set_coverage",
            {
        ...
        bad_cat = _run_api_context_tool(
            coll.id,
            run.id,
            "set_coverage",
            {
                "endpoint_id": ep.id,
                "owasp_api_category": "API99",
                "status": "covered",
            },
        )
        assert "error" in bad_cat

        bad_ep = _run_api_context_tool(
            coll.id,
            run.id,
            "set_coverage",
            {
        ...
        not_applicable = _run_api_context_tool(
            coll.id,
            run.id,
            "set_coverage",
            {
...
def test_start_alice_run_endpoint_uses_api_run_type(client):
    ...
    async def _fake_start(
        run_id, *, tab_id, think_msg_id, reply_msg_id, message, history, run_type="site"
    ):
    ...
    try:
        ...
        r = client.post(
            "/api/api-collections",
            json={"name": "Z", "base_url": "https://z.example.com"},
        )
        cid = r.json()["id"]
        run = client.post(
            f"/api/api-collections/{cid}/test-runs", json={"name": "t"}
        ).json()
        rid = run["id"]

        resp = client.post(
            f"/api/api-test-runs/{rid}/alice/run",
            json={
                "message": "test",
                "history": [],
                "tab_id": "tab-default",
