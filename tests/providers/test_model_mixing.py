...
def test_role_override_beats_default(db_session: Session):
    ...
    prof = settings_svc.create_scan_profile(
        db_session,
        LLMProfileIn(
            name="P", default_model_id=smart.id, role_models={"crawler": cheap.id}
        ),
    )
    run = SimpleNamespace(llm_profile_id=prof.id, llm_config_id=None)

    assert (
        settings_svc.get_llm_config_for_role(db_session, run, "crawler").id == cheap.id
    )
    # Unmapped role → default model.
    assert (
        settings_svc.get_llm_config_for_role(db_session, run, "test_lead").id
        == smart.id
    )
...
def test_unassigned_mentor_inherits_test_lead_before_profile_default(
    db_session: Session,
):
    cheap = _mk_model(db_session, "cheap")
    smart = _mk_model(db_session, "smart")
    prof = settings_svc.create_scan_profile(
        ...
        LLMProfileIn(
            name="P",
            default_model_id=cheap.id,
            role_models={"test_lead": smart.id},
        ),
    )
    run = SimpleNamespace(llm_profile_id=prof.id, llm_config_id=None)

    assert (
        settings_svc.get_llm_config_for_role(db_session, run, "mentor").id == smart.id
    )
...
def test_no_profile_falls_back_to_legacy_config(db_session: Session):
    ...
    assert (
        settings_svc.get_llm_config_for_role(db_session, run, "crawler").id == legacy.id
    )
...
def test_active_profile_used_when_run_unset(db_session: Session):
    ...
    settings_svc.create_scan_profile(
        ...
        LLMProfileIn(
            name="active",
            default_model_id=smart.id,
            role_models={"validator": cheap.id},
        ),
    )  # first profile auto-activates
    run = SimpleNamespace(llm_profile_id=None, llm_config_id=None)
    assert (
        settings_svc.get_llm_config_for_role(db_session, run, "validator").id
        == cheap.id
    )
    assert (
        settings_svc.get_llm_config_for_role(db_session, run, "crawler").id == smart.id
    )
...
def test_falls_back_to_active_model_without_profiles(db_session: Session):
    ...
    assert (
        settings_svc.get_llm_config_for_role(db_session, run, "crawler").id == only.id
    )
...
def test_profile_rejects_unknown_model_and_role(db_session: Session):
    ...
    with pytest.raises(Exception):
        settings_svc.create_scan_profile(
            ...
            LLMProfileIn(
                name="bad-role",
                default_model_id=good.id,
                role_models={"crawler": 99999},
            ),
        )
    with pytest.raises(Exception):
        settings_svc.create_scan_profile(
...
def test_single_active_profile(db_session: Session):
    ...
    p1 = settings_svc.create_scan_profile(
        db_session, LLMProfileIn(name="p1", default_model_id=a.id)
    )
    p2 = settings_svc.create_scan_profile(
        db_session, LLMProfileIn(name="p2", default_model_id=a.id)
    )
...
def test_run_persists_llm_profile_id(client: TestClient):
    ...
    bad = client.post(
        ...
        json={
            "max_depth": 2,
            "max_pages": 10,
            "llm_profile_id": 99999,
        },
    )
    assert bad.status_code == 404


# --- Merged from test_openrouter_provider.py ---
