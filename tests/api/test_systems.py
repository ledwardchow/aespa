"""Systems API: CRUD, same-system validation, and safe ZIP
...
def _create_system(client, name="Acme Portal") -> dict:
...
def _create_component(client, app_id, name="checkout-ui") -> dict:
    resp = client.post(f"/api/systems/{app_id}/components", json={"name": name})
    assert resp.status_code == 201, resp.text
    return resp.json()
...
def _upload_snapshot(client, app_id, component_id, filename="v1.zip") -> dict:
    resp = client.post(
        f"/api/systems/{app_id}/components/{component_id}/snapshots",
...
# ── System / component CRUD ─────────────────────────────────────────────


def test_create_and_get_system(client):
    app = _create_system(client)
    resp = client.get(f"/api/systems/{app['id']}")
    assert resp.status_code == 200
    assert resp.json()["name"] == "Acme Portal"


def test_duplicate_system_name_rejected(client):
    _create_system(client, "Dup")
    resp = client.post("/api/systems", json={"name": "Dup"})
    assert resp.status_code == 409


def test_component_names_unique_within_system(client):
    app = _create_system(client)
    _create_component(client, app["id"], "orders-api")
    resp = client.post(
        f"/api/systems/{app['id']}/components", json={"name": "orders-api"}
    )
    assert resp.status_code == 409


def test_component_names_can_repeat_across_systems(client):
    app_a = _create_system(client, "App A")
    app_b = _create_system(client, "App B")
    _create_component(client, app_a["id"], "shared-name")
    resp = client.post(
        f"/api/systems/{app_b['id']}/components", json={"name": "shared-name"}
    )
...
def test_snapshot_upload_creates_new_immutable_version(client, tmp_path, monkeypatch):
    ...
    resp = client.get(
        f"/api/systems/{app['id']}/components/{component['id']}/snapshots"
    )
    assert resp.status_code == 200
...
def test_snapshot_upload_enforces_size_limit(client, tmp_path, monkeypatch):
    ...
    assert systems_svc.MAX_SNAPSHOT_UPLOAD_BYTES == 250 * 1024 * 1024
    monkeypatch.setattr(systems_svc, "MAX_SNAPSHOT_UPLOAD_BYTES", 4)
    monkeypatch.setattr("aespa.api.systems._MAX_SNAPSHOT_UPLOAD_BYTES", 4)
    app = _create_system(client)
    component = _create_component(client, app["id"])
    resp = client.post(
        f"/api/systems/{app['id']}/components/{component['id']}/snapshots",
...
def test_delete_snapshot_blocked_while_referenced_by_campaign(
    ...
    campaign_resp = client.post(
        f"/api/systems/{app['id']}/campaigns",
        json={
            "name": "release-1",
            "source_members": [
    ...
    delete_resp = client.delete(
        f"/api/systems/{app['id']}/components/{component['id']}"
...
def test_campaign_detail_reports_linked_child_run_status(
    ...
    response = client.get(f"/api/systems/{app['id']}/campaigns/{campaign['id']}")
...
# ── Same-system validation ──────────────────────────────────────────────


def test_campaign_rejects_component_from_another_system(
    ...
    site_id = site_resp.json()["id"]
    target_resp = client.post(
        f"/api/systems/{app_a['id']}/targets",
        json={"target_type": "site", "target_id": site_id},
    )
    target = target_resp.json()

    resp = client.post(
        f"/api/systems/{app_a['id']}/campaigns",
        json={
            "name": "cross-app",
            "source_members": [
...
def test_hint_rejects_target_from_another_system(client, tmp_path, monkeypatch):
    monkeypatch.setenv("AESPA_DATA_DIR", str(tmp_path))
    app_a = _create_system(client, "App A")
    app_b = _create_system(client, "App B")
    ...
    site_id = site_resp.json()["id"]
    target_resp = client.post(
        f"/api/systems/{app_b['id']}/targets",
        json={"target_type": "site", "target_id": site_id},
    )
    target_b = target_resp.json()

    resp = client.post(
        f"/api/systems/{app_a['id']}/hints",
...
def test_attach_and_detach_target(client, isolated_db_engine):
    ...
    detach = client.delete(f"/api/systems/{app['id']}/targets/{target['id']}")
    assert detach.status_code == 204
    with Session(isolated_db_engine) as s:
        assert s.get(SystemTarget, target["id"]) is None
...
def test_target_can_optionally_select_code_component(client):
    ...
    update = client.patch(
        f"/api/systems/{app['id']}/targets/{target['id']}",
...
def test_target_component_link_rejects_component_from_another_system(client):
    app_a = _create_system(client, "Target owner")
    app_b = _create_system(client, "Component owner")
    ...
    update = client.patch(
        f"/api/systems/{app_a['id']}/targets/{target['id']}",
        json={"component_id": component_b["id"]},
    )
    assert update.status_code == 400


def test_system_delete_blocked_while_campaign_exists(
    client, tmp_path, monkeypatch
):
    monkeypatch.setenv("AESPA_DATA_DIR", str(tmp_path))
    app = _create_system(client)
    ...
    resp = client.delete(f"/api/systems/{app['id']}")
    assert resp.status_code == 409
...
def test_review_endpoint_accepts_empty_decisions_with_zero_proposals(
    ...
    resp = client.post(
        f"/api/systems/{app['id']}/campaigns/{campaign['id']}/review",
        json={"decisions": []},
    )
...
def test_review_endpoint_rejects_unknown_mapping_id(client, isolated_db_engine):
    ...
    from aespa.db import get_engine
    from aespa.models import AssessmentCampaign, System

    with Session(get_engine()) as s:
        app_row = System(name="ReviewApp")
        s.add(app_row)
        s.flush()
        campaign = AssessmentCampaign(
            system_id=app_row.id, name="c", status="awaiting_review"
        )
        s.add(campaign)
        s.commit()
        app_id = app_row.id
    ...
    resp = client.post(
        f"/api/systems/{app_id}/campaigns/{campaign_id}/review",
