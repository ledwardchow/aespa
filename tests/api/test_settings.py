...
def test_component_mapper_config_round_trip(client: TestClient):
    ...
    updated = client.put(
        "/api/settings/component-mapper-config",
        json={
            "max_tool_calls": 180,
            "max_source_files": 750,
    ...
    assert updated.status_code == 200
    assert updated.json()["max_tool_calls"] == 180
    assert updated.json()["max_source_files"] == 750
    assert updated.json()["max_source_bytes"] == 75 * 1024 * 1024
    assert updated.json()["max_facts"] == 300
    assert updated.json()["max_concurrent"] == 6
