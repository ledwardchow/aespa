...
def test_start_endpoints_accept_sast_validate_and_require_open_leads(
    ...
    web_lead = _imported_lead(
        isolated_db_engine, run_type="web", run_id=web_run.id
    )
...
def test_sast_validate_prompt_and_tools_are_focused():
    ...
    assert "browser" in {tool["name"] for tool in get_sast_validate_tools(is_api_run=False)}
    assert "browser" not in {tool["name"] for tool in get_sast_validate_tools(is_api_run=True)}
