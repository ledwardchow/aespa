...
def test_poc_request_expect_requires_positive_assertion():
    ...
    assert (
        validator.poc_request_expect({"poc_expect": {"body_contains": "x"}}) is not None
    )
...
def test_resolve_poc_auth_returns_none_when_credential_absent():
    assert (
        validator._resolve_poc_auth({"extra_headers": {}, "cookies": {}}, "bearer")
        is None
    )
    assert (
        validator._resolve_poc_auth(
            {"extra_headers": {}, "cookies": {}}, "cookie_httponly"
        )
        is None
    )
...
def test_run_and_assert_curl_honours_status_mismatch(monkeypatch):
    ...
    assert (
        validator._run_and_assert_curl(["curl"], {"status": 200, "body_contains": ""})
        is False
    )
...
def test_build_and_verify_poc_includes_setup_for_auth(monkeypatch):
    ...
    sessions = {
        "victim": {"username": "victim", "cookies": {"sid": "abc"}, "extra_headers": {}}
    }
    result = asyncio.run(
...
def test_post_method_is_accepted(monkeypatch):
    ...
    result = asyncio.run(validator._build_and_verify_poc(finding, done, {}, _Policy()))
    assert result is not None
    command, _ = result
