...
| 7 | Stored XSS in account-detail transaction table | ✗ | ◐ Adjacent stored XSS finding, wrong sink | ◐ Generic possible XSS only | ✗ | ✓ |
...
| 18 | JWT signature bypass | ◐ Identified admin JWT token expiry not validated; did not independently confirm the signature-ignore flaw | ✗ | ✗ | ◐ Found JWT forgery via leaked secret rather than the signature-ignore flaw | ✓ |
...
|---|---:|---:|---:|---:|
| Aespa Sonnet 4.6 | 13 | 1 | 9 | 8.5 / 10 |
...
Aespa produced the broadest confirmed finding set of any scanner in this comparison. It identified 13 of 23 ground-truth vulnerabilities outright, including both SQL injection instances, all three A01 IDOR findings, the TOTP bypass, the balance-check bypass, permissive CORS, `/api/health` JWT-secret disclosure, login enumeration, stack-trace leakage, MD5 hashing, and sensitive API data exposure.

The most notable improvement over other scanners was dynamic confirmation of the dashboard stored XSS — the only scanner to move that finding from a static-analysis partial to a fully confirmed exploit. It also uniquely identified the SQLi in the transaction `sort` parameter (#22) and confirmed the no-balance-check logic flaw (#9) as a standalone vulnerability rather than an incidental observation.

Its primary weakness is signal-to-noise ratio. Fifty-nine raw findings collapsed to 26 unique issues, a 56% duplication rate driven by the validator generating parallel re-confirms of the same vulnerability across different severity buckets. This significantly increases triage effort. The scanner also missed brute-force protection (#17), no-audit-logging (#20), the account-detail XSS sink (#7), and the full A06 and A10 categories — consistent with a black-box dynamic-only approach that did not inspect dependencies or probe the avatar endpoint.
...
|---|---|
| Aespa Sonnet 4.6 | Weak admin credentials; IDOR on admin API allowing direct balance manipulation; admin account inventory exposure without scoping; unauthorized read access to other users' profile data; admin JWT token expiry not validated; TOTP setup endpoint accessible without authentication; deleted/invalidated tokens accepted by some endpoints; additional stored XSS sink (address-book nickname field); missing Content-Security-Policy header; admin panel accessible without network restriction; server-version disclosure. |
...
1. **Aespa Sonnet 4.6** — best overall coverage (13/23), only scanner to dynamically confirm dashboard XSS, and uniquely found the balance-check bypass and transaction `sort` SQLi.
